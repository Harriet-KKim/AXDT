...
# ADR-0008: Git 호스트는 어댑터+백엔드 합성으로 추상화하고 게이트 재개는 리뷰 커서로 판정한다

## 상태
Accepted (2026-07-07) · 관련 결정 D5

0006은 Phase 3(git 격리) 예약이다. 강제(호스트 브랜치 보호)는 이 클라이언트 증분과 별개의 후속 증분이며, 0007 확장 대신 신규 `ADR-0009`로 번호를 뗀다(0007은 Phase 3 세션이 정식화 중이라 브랜치 편집 충돌을 피한다).
