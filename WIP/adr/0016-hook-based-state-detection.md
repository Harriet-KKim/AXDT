...
date: 2026-07-21
decision: D4
related: [ADR-0005, ADR-0018, ADR-0019]
---
...
## 결정
- `poll_state`/`detect_state`를 화면 마커 스캔에서 "CLI 훅이 쓴 상태 파일 읽기"로 바꾼다. 훅이 전이 시점에 한 줄 JSON `{"state", "ts"}`를 원자적으로 쓰고(`PLATFORM_MATRIX` '상태 파일 계약' 절), 백엔드가 읽는다. `detect_state`는 추출된 상태값(`idle`·`busy`·`waiting_input`·`starting`)을 `AgentState`로 매핑한다(값이 `AgentState.value`와 항등 — 어휘 통일은 ADR-0019).
