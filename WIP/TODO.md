...
- [ ] **SoT PR 2건 (사용자 게이트)** — ① `rule-role-responsibilities`(역할→쓰기경로 단일 명세) 신설 + `rule-protected-paths` 개정(경로축/역할축 분리·상호참조 · **ADR 작성권 Leader→Maintainer 이동**, 사용자 결정): **작성·2모델 리뷰 수렴, ADR 변경 6파일+계약테스트 적용, 최신 main 재조정 완료**(role-responsibilities `scope` global→local(ADR-0015)·protected-paths는 main SoT 프레임워크 위에 얹음, 병합 `33ea79b`, 646 테스트 통과). **PR #10을 사용자 게이트로 확정**(별도 `sot/` 브랜치 안 씀 — 재조정 후 SoT 변경이 #10에 정합) → 사용자 승인·병합 대기 (2026-07-16). ② `rule-prompt-injection`(주입 규약)은 메시지 계약(§4.1 message)만 골격 확정, 게이트 배선은 Phase 5
