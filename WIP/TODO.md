...
- [x] **상태판정 재설계 — 훅 기반** (슬라이스 A ①, §8.3a 발견 인계) — `detect_state`/`poll_state`를 화면 긁기에서 훅이 쓴 상태파일 읽기로 교체, 화면 마커 폐기, 상태 파일 형식 `{state,ts}`(`PLATFORM_MATRIX` '상태 파일 계약' 절) 확정. claude·codex 훅(SessionStart→idle·UserPromptSubmit→busy·Stop→idle)→상태. `ts` 역행 가드는 슬라이스 B 유보. 근거·실측 → `WIP/handoff-state-detection-redesign.md` — **phase5-runtime 브랜치 완료(미병합), 886 passed**
