...
- [x] **[높음] 용어집(glossary) 작성** — AXDT 설계 전반의 용어를 한곳에 정의. SoT/Interim 정도만 `terminology.md`·본 TODO에 흩어져 있어 Maintainer·Leader·게이트·readiness·finding(`F-n`)·`review_clear`/`accepted`/`rejected`·트리 해시 등의 단일 사전이 없었다. ✅ 2026-07-09 → `docs/glossary.md` — 위치·형식은 **비SoT 사람용 guide, `docs/` 직속**으로 확정(6그룹·78항목, 각 항목에 rule/ADR/TODO 출처 링크. 권위는 원본에 두고 링크만).
- [x] **[높음] 문서 워크플로 도식화** — SoT·interim 각 문서의 생애와 문서 간 관계를 사람이 한 눈에 이해할 도식으로. 통신 채널 맵·상태 모델(report→progress)·디렉터리 구조·강제 계층이 여러 문서에 흩어져 통합 자료가 없었다. ✅ 2026-07-09 → `docs/workflow.md` — Mermaid 2레인(SoT 완료 판정 / interim 개발 루프) + 문서별 작성자·상태 표 + 강제 3층(D15) + 개발·테스트 SCAFFOLD. HTML Artifact 뷰 병행.
- [ ] **[중간] tmux → herdr 전환** — herdr는 tmux처럼 워크스페이스·탭·pane을 지속하면서, pane에서 도는 AI 에이전트를 감지해 상태(blocked/working/done/idle)를 사이드바로 노출하는 "에이전트 인식" 멀티플렉서(Rust)다. Maintainer가 다수 Leader의 진행 상태를 tmux로는 직접 관찰하지 못하는 지점을 네이티브로 메울 수 있다. 다만 tmux는 D1(Maintainer=상시 세션)·D2·`ADR-0003`(send-keys 하향 주입)·`ADR-0001`의 전제이고 Phase 3(오케스트레이션)·5(agent runner)·7(메신저 inbound→send-keys)에 걸쳐 있어, 채택은 새 결정(D17)과 ADR 갱신을 수반한다. **선결 확인** ① herdr에 `send-keys` 상당의 프로그래매틱 prompt 주입·출력 읽기 경로가 있는가 — 없으면 D2 통신 모델이 성립하지 않는다. ② herdr의 에이전트 상태는 관찰용 보조 신호로만 쓴다 — 상태 권위는 `progress.status`(`ADR-0004`)이므로 경쟁 상태원을 만들지 않는다. **리스크**: 프로젝트가 젊고(2026-07 기준 약 105일) 단일 개발자·빠른 변경이라 breaking change 가능. 참고: https://herdr.dev · https://github.com/ogulcancelik/herdr
- [ ] **[높음] 스크립트로 가능한 일을 Agent에 위임하고 있지 않은지 감사** — 결정적으로 스크립트가 처리할 수 있는 작업을 LLM Agent에게 맡기고 있지 않은지 전면 검토한다. 목적은 **LLM 토큰 소모 감축**과 결정성·재현성 확보다(같은 입력에 같은 결과). `rule-sot-readiness`가 이미 "① 형식 = 기계 검증 / ② 정합성·공백 = LLM 검토"로 절단선을 그었으니, 그 원칙을 시스템 전체로 확장하는 작업이다. **점검 후보**: progress 테이블 파싱·갱신(D7 엄격 스키마는 애초에 기계 파싱 전제)·report→progress 집계·wave 롤업, workspace/컨테이너 생성·삭제 glue(Phase 3), 네이밍·보호 경로 검증(게이트), 커버리지 표 생성·감사 로그 포맷팅, 트리 해시·적용 rule 지문 계산. **LLM에 남겨야 할 것도 명시**한다 — 의미 판단(② 검토), 회색지대 결정, wave/task 분해. **산출**: 위임 지점 목록 + 스크립트 이관 후보 + 예상 토큰 절감. **착수 시점**: Phase 8(오케스트레이션 엔진)이 굳기 전 — 늦으면 비싼 LLM 호출이 엔진에 박힌 뒤라 재작업이 된다.
