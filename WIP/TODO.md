...
>
> 작성일: 2026-06-26 · 갱신: 2026-07-14 (Phase 6 "SoT 완료 강제" 서브트랙 추가 · ADR 0008~0010 등재 · D15 참조 갱신) · 상태: 초안
...
- [x] **설계 확정** — 설계서 3종(사양서 · `ADR-0009` · 완료 판정 규칙 `sot-readiness`) 작성·동결
- [ ] **판정 로직 구현 (GitHub 접속 없이 계산되는 핵심부)** — "이 PR을 병합해도 되는가"를 입력값만으로 초록/빨강 판정하는 핵심 함수 + 검토 지적을 서로 같은 것으로 대조하는 '지문(해시)' 계산 함수 (`WIP/axdt/sot_gate/`)
...
- [ ] **[중간] 규칙 파일 `scope:` frontmatter 채우기** — `docs/sot/rule/` 규칙 9개의 `scope`(전파 범위) 필드가 비어 있어, 각 규칙이 어느 프로젝트·경로에 적용되는지 메타로 표시되지 않는다. 채워 넣는다.
