...
- [x] **역할별 실행** (슬라이스 A ③) — `capability_args`·`prepare_subagents`(Claude `--agents`)·`build_session_command`(Claude `--append-system-prompt` / Codex `-p <role>` 프로파일 바인딩 — 역할 프롬프트는 프로파일이 얹음, 표면·계층·fail-closed는 Phase 3 실측), `start_session(role, …)`. 다중모델 3R + 역할 전달 결정 재리뷰 3R 수렴 — **phase5-runtime 완료(미병합)**
