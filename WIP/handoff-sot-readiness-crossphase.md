...
> **RESOLVED (2026-07-07, main 병합 시점).** §3 귀속 충돌은 (A) Phase 6 흡수로 정합화됐고, 병합된 `docs/sot/rule/sot-readiness.md`의 강제 매핑 표(호스트 브랜치 보호 = Phase 6)와 phase6 스펙 §9 "귀속(Phase1 정합): 호스트 브랜치 보호 강제 = Phase 6"이 일치한다. §1 전문의 "현재 Phase 6 설계는 강제 = Phase 3이라 적었다"는 스테일(정합화 완료). **잔여(비차단):** §1의 강제 층 자체(①②③ + 감사 이력 보존)는 아직 미구현 — Phase 6 강제 증분(`ADR-0009`, 2026-07-09 설계 완료·proposed)으로 남는다. 그 증분은 필수 검사가 아니라 **머지 컨트롤러**(`main` 갱신 권한을 지정 신원으로 제한 + 머지 직전 판정)로 강제한다. 아래 본문은 역사 기록으로 보존한다.
>
