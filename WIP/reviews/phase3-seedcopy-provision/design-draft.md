# §8.3b 후속 — seed+copy 운영 설계와 §4.1 반영 (검토용 초안 v10)
...
| ID | 미측정 질문 | 좌우 결정 | 우선순위 |
|---|---|---|---|
| **R2** | claude가 refreshToken 없이 **accessToken만**으로 세션 동작? | 3.1 지급형식 | **요구 blocker** |
| **R14c** | **재시작 복구(감독자 A 형상)** — 호스트↔감독자 핸드셰이크(stop→push ack→신선도 검증→재기동/거부)로 fresh AT 기입 후 claude 재기동(기동 시 재독) + `--continue` 복원. *형제 게이트 R14c-전달(push 권한·동기화·부분 주입) / R14c-복원(--continue) 분리* | 3.1/3.2/3.3 바닥 | **요구 blocker** |
| **R15** | 만료 임박 시 호스트 **정품 claude 저비용 호출이 갱신 유발**하는가 — **바닥도 fresh AT 공급 전제** | 3.1 갱신기 | **요구 blocker** |
| R14c′ | **재생성 복구(B 폴백 형상)** — 컨테이너 재생성 + 영속 트랜스크립트(또는 오케스트레이터 복원) + `--continue` | 3.1/3.3 폴백 | 폴백 게이트 |
| R1 | refresh 시 refreshToken **회전**? 재사용 탐지 → 패밀리 **revoke**? | 3.1 회전 | 회전 |
| R4 | **동일 accessToken을 N개 컨테이너 동시 사용** 시 서버측 충돌·이상탐지? | 3.1/3.4 | 동시성 |
| R8 | refresh가 **구 AT 무효화** / **단일 활성 AT** 정책? (동시성·선제/반응·다중세션 바닥·**공급단 세대** 결정) | 3.1 | 회전·동시성·**함대 토폴로지 요구-임계** |
...
> **측정 완료(§1 편입)**: R3·R7a·R9(파일마운트 부팅 한정)·R10. R13은 필요성 실증(전달 수단 확정은 미측정).
> **요구 blocker = R2 · R14c(전달+복원) · R15.** R14/R14d는 무재시작 hot-handoff 최적화. R8은 다중세션 바닥·공급단 세대·동시성 결정.
...
- **⚠ 공급단(갱신 구동기)의 세대 결속(R8=참 분기)**: 갱신 구동기가 **사용자 호스트 로그인**이면 컨테이너와 **같은 refreshToken 패밀리**다. R8=참이면 **사용자의 호스트 claude 사용이 새 세대를 낳아 컨테이너 accessToken을 무효화**한다(전역 세대 전환 프로토콜은 사용자의 즉흥적 호스트 사용을 정렬 못 함). → R8=참이면 구동기를 **사용자와 분리된 전용 계정**으로 두거나 컨테이너별 격리 로그인. **"RT 만기(~29d)를 사용자 호스트 상용으로 흡수" 이점은 R8=거짓 전제**.
- **원자적 저장**: RT 회전 시 "새 RT 수신 성공·저장 실패"는 계정 잠금급 치명.
...
**위험(회전·공유) 및 R8=참 분기.**
- **IF** refresh가 refreshToken 회전(R1)·공유: 한 컨테이너 refresh가 공유 RT 무효화·재사용 탐지 시 패밀리 revoke(RFC 9700). RT는 고가치 표적.
- **R8=참(구 AT 무효화):** 새 AT(세대 G2) 발급이 구 G1을 즉시 무효화.
  - **바닥은 단일 컨테이너에만 견고**: 한 컨테이너가 자기 만료 경계에서 새 세대로 재시작하는 것은 성립(짧은 재시작이 유일 중단).
  - **⚠ 다중 컨테이너 공유 AT는 바닥으로 불충분**: 한 컨테이너의 새 세대 발급이 아직 G1을 쓰는 **다른 컨테이너를 끊는다**. R8=참이면 공유 AT 운영은 **전역 세대 전환 프로토콜**(모든 공유-계정 컨테이너를 세대 경계에 정렬) 또는 **컨테이너별 격리 로그인**이 필요. 공급단(위)까지 포함해 R8=참이면 **단일 계정 공유가 근본적으로 취약**. **AXDT 정상 토폴로지(workspace마다 컨테이너 1개를 사용자 한 계정으로)에선 형제 컨테이너 롤오버가 동족 살해**가 되므로 R8은 함대 토폴로지에서 **요구-임계 측정** — 무거운 구현 전에 측정하고, R8=참이면 전역 세대 전환 프로토콜 상세화 또는 격리 로그인 아키텍처 전환이 필요함을 발의 경고로 싣는다(프로토콜 선설계는 R8=거짓이면 낭비라 R8 뒤로). → **R8/R4 결과로 게이트**.
...
**과승격 금지.** 실측은 Claude만. Codex `trust_level`(R5)·node(R6) 미해결 → **"Claude 한정 잠정"**. Codex arm·비잠정화는 R5/R6 및 잔여 게이트(R2·R14c·R15 등) 종료 후.
...
- **맥락**: §4.1은 신뢰·온보딩을 이미지에 굽고 자격증명만 런타임 주입(:672). 실측으로 항목 11-13 + R3/R7a/R9(파일마운트 부팅)/R10/R13(필요성). **장기세션 요구 = "세션 지속(짧은 재시작 허용)"**(사용자) → 재시작 롤오버가 요구 충족, hot-handoff는 최적화.
- **결정**:
  ...
  4. **토큰 수명 = 중앙 lease 갱신기**(정품 claude 구동·원자적 저장, RT 중앙 한 곳). **요구 충족 = 재시작 롤오버(R14c, 전달·복원·공급 포함)**; hot-handoff(R14/R14d)는 최적화. **R15는 공급 전제 요구 게이트**(실패 시 폴백·사람 개입 주기 명시). R8=참이면 갱신 반응형·최적화는 R14d 특정·**공급단 구동기도 같은 세대라 전용 계정 분리 필요**·공유 AT 다중 컨테이너는 전역 세대 전환/격리 로그인(R8/R4 게이트).
...
- **결과(consequence)**:
  ...
  - **측정 게이트**: seed provenance+버전 일치. **요구 blocker = R2·R14c(전달+복원)·R15**; **R14c-복원(`--continue`)은 A·B 공통 급소로 최우선**. 최적화 = R14/R14d. **R8은 함대 토폴로지(한 계정 공유 N 컨테이너) 요구-임계** — R8=참이면 단일-공유-계정 전제가 뒤집힐 수 있음(발의 경고).
...
- **미해결(측정 게이트)**: R1·R2·R4·R5·R6·R7b·R8·R11·R12·R12′·R13·R14·R14c·R14c′·R14d·R15 + redact/PII. 종료 전 값 동결·비잠정화 금지.
- **인용·병합**: 스펙·`live_probe*`는 phase2 브랜치 전용. ADR main 착지 시 병합 선후 명시.

## 5. 이행 순서(제안)

1. **요구 blocker 우선(가장 싸게 무효화 가능한 것부터)**: **R14c-복원(`--continue`)** — 감독자·재생성 공통 급소, **최우선**. **R2**(accessToken-only 동작), **R14c-전달**(push 핸드셰이크·권한·부분 주입), **R15**(갱신 유발·공급). **R8**(구 AT 무효화·다중세션 바닥·공급단 세대 — **함대 토폴로지 요구-임계, 무거운 구현 전**)·R1(회전), R4(N-컨테이너 동시). 최적화 게이트 R14(재독+expiresAt 하위)·R14d(apiKeyHelper). **감독자 호환성 계약 테스트**(placeholder/pytest 비감독 exec, `sh -c` 종료코드·신호·stdout/stderr·tmux 관측, `send_text`/`read_new_output` 재시작 창 거동, `--entrypoint` 우회). R7b·R12′(tmpfs·swap)·R13(전용 GID)·R14c′(재생성+영속 트랜스크립트)·R11(PII). R5·R6은 별도. — redact/PII.
