...
- 결정권자 명단의 저장 위치 — 저장소 안에 둘 경우에만 포함. 저장소 밖(컨트롤러 도메인)에 두면 해당 없음. 이 위치 자체가 Phase 6 미결(리뷰 발견 7)이다.
...
- 결정권자 명단의 저장 위치(저장소 안/밖) — 리뷰 발견 7, 반영 진행 중.
- 게이트·컨트롤러 코드의 정식 이관 위치와 배포 pin — sub-spec §8.
