...
- **agent_runner 계약의 infra 통합.** 슬라이스 A(①②③)가 `agent_runner`에 정의·착지시킨 계약을 infra에 잇는다. Phase 5 세션에서 하지 않은 이유: 전부 컨테이너/이미지 지식이 필요하다.
  ...
  - **Codex 세션 역할 프롬프트 = `-p <role>` 프로파일이 얹는 외부 아티팩트(후보 — 실측 미완).** 역할 텍스트 SoT는 `docs/sot/rule/role-responsibilities.md`이고 `roles/prompts/<role>.md`가 담는다. `CodexAdapter.build_session_command`이 `-p <role.name>`을 실어 역할↔프로파일을 바인딩한다(스펙 line 155·§2.3.3). **`-p`는 결정적 바인딩일 뿐 부재 프로파일에 대한 자체 실패 보장은 아니다** — Codex 0.144.3은 프로파일이 없어도 오류 없이 기본값으로 진행하므로(실측), fail-closed 강제는 아래 미결 4가 Phase 3에 맡긴다. Phase 3가 `$CODEX_HOME/<role>.config.toml`을 물질화한다. 이 방식은 `up`(start_session)↔`attach`(assign/send) 프로세스 분리가 낳던 부트스트랩 타이밍 문제를 없앤다.
    - **미결 — Phase 3/슬라이스 B가 실측·배선한다(확정 아님).**
      1. **역할 프롬프트를 얹을 정확한 네이티브 표면.** `$CODEX_HOME`/전역 `AGENTS.md` 자동 로드는 그 내용이 **`user` 계층**에 들어가 workspace `AGENTS.md`가 의미적으로 override할 수 있다(Codex 0.144.3 `debug prompt-input` 관측) — 역할 정체성 계층(`developer`)이 아니다. 지운 런타임 주입(첫 타이핑 프롬프트)도 마찬가지로 `user` 계층이었다. 유력 후보는 프로파일의 `developer_instructions`(또는 검증 후 `model_instructions_file`) — 대상 Codex 버전에서 입력 계층·override·재시작 로드를 실측해 확정한다. 스펙 §CLI표(line 151)의 "본문 선두 주입" 전제는 이 결정으로 대체되므로 phase2 스펙 소유자가 갱신한다.
      2. **Maintainer는 호스트다(이미지 아님).** Leader는 컨테이너 이미지에 굽지만 Maintainer는 호스트에서 도므로 "이미지 빌드 시 $CODEX_HOME 굽기"로 안 된다 — 호스트 물질화 경로를 별도로 배선한다.
      3. **보호.** tamper 방지는 `rule-protected-paths`가 아니라(그건 리포 push 경로 규칙이라 `$CODEX_HOME` 무관) 이미지·호스트의 **읽기전용 굽기 + OS 소유권/권한**으로 한다.
      4. **수용 게이트 + fail-closed 강제.** `-p`만으로는 Codex가 부재 프로파일을 조용히 넘기므로, Phase 3(leader.up/이미지 빌드)가 기동 전 `$CODEX_HOME/<role>.config.toml` 존재·역할 일치를 검사해 부재·불일치면 `backend.start()`를 하지 않는다. 버전 고정 라이브 수용 테스트가 ① 역할별 프롬프트가 실제 `developer` 계층으로 로드됨 ② 프로파일 부재 시 기동 거부(음성 케이스)를 모두 통과해야 이 방식을 확정한다.
