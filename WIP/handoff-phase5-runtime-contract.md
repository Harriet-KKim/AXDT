...
> 상태: 확정 · 슬라이스 A 완료(phase5-runtime 브랜치, 미병합). 다중모델 리뷰로 수렴 — 원 슬라이스 A 3R + 역할 전달 결정 재리뷰 3R + 역할·권한 보증 계약 재리뷰 3R. agent_runner 108 passed · WIP 전체 923 passed. 아래 phase2·Phase 3 인계는 유효하다.
...
> **정본 이관됨.** 이 계약의 단일 진실원은 `axdt/agent_runner/PLATFORM_MATRIX.md`의 '상태 파일 계약' 절이다. 아래는 배경 서술이며, 계약 세부(경로·형식·원자적 쓰기·순서 정책·상태값→`AgentState` 매핑)는 그 절을 따른다(상태값 매핑의 정본은 매트릭스 '훅 이벤트 → 상태' 절·`_STATE_MAP`, 파일 부재 처리는 그 절 부재 행의 잠정(슬라이스 A: `STARTING` 유지)/목표(ADR-0018: 부재→`ERROR`) 구분을 따른다). 이 핸드오프가 삭제돼도 계약은 그 절에 남는다.

훅이 상태를 파일에 쓰고, 백엔드(호스트)가 그 파일을 읽는다. 파일 형식은 **Phase 3(쓰는 쪽, 훅 명령)와 Phase 5(읽는 쪽, poll_state)의 공유 계약**이다. 아래 항목은 이관 전 배경 서술이며, 규범 정본은 위 배너가 가리키는 매트릭스 절이다.
...
- **형식.** 한 줄 JSON: `{"state": "<상태>", "ts": <epoch 초, 소수 포함>}`. `<상태>`는 `starting`·`idle`·`busy`·`waiting_input` 중 하나(값이 `AgentState.value`와 항등, ADR-0019).
...
- **상태값→`AgentState` 매핑(poll_state 내부).** `idle`→`IDLE`(SessionStart는 "수신 준비됨"이므로 `IDLE`), `busy`→`BUSY`, `waiting_input`→`WAITING_INPUT`, `starting`→`STARTING`. 값이 `AgentState.value`와 항등이라 별도 별칭은 없다(ADR-0019). `STARTING`은 seed된 `starting` 값 또는 (seed 배선 전) 파일 부재 시 유지로 관측된다. `ERROR`·`STOPPED`는 파일이 아니라 프로세스 생존으로 판정(§2) — `PLATFORM_MATRIX` 훅 이벤트표와 정합.
...
- **`message.py` `render_note` 제약 — 근거 소멸(정리는 phase2 판단).**
  - 증상: `render_note` docstring(97-100행)과 스펙 §4.1(591·717행)·§2.4(229행)·§11(1331·1353행)이 "자유 텍스트에 `Error:` 같은 마커가 섞이면 `detect_state`가 transcript에서 찾아 판정이 오염된다"를 근거로 제약을 건다.
...
- **agent_runner 계약의 infra 통합.** 슬라이스 A(①②③)가 `agent_runner`에 정의·착지시킨 계약을 infra에 잇는다. Phase 5 세션에서 하지 않은 이유: 전부 컨테이너/이미지 지식이 필요하다.
  ...
  - **Codex 세션 역할 프롬프트 = `-p <role>` 프로파일 + 어댑터가 계산·보증하는 역할 아티팩트(ADR-0017).** 역할 규범(책임·권한)의 SoT는 `docs/sot/rule/role-responsibilities.md`, 프롬프트 문구의 정본은 `roles/prompts/<role>.md`가 담는다(roles/spec.py 계층). `CodexAdapter.build_session_command`이 `-p <role.name>`을 실어 역할↔프로파일을 바인딩하고(스펙 line 155·§2.3.3), **`role_artifacts(role, root)`가 그 프로파일(`<role>.config.toml` — `sandbox_mode`·`developer_instructions`를 최상위 키로, `model`은 `model_hint` 있을 때만)을 계산해 역할·권한 내용의 단일 진실원이 된다.** `-p`만으로는 부재 프로파일에도 Codex 0.144.3이 기본값으로 진행하므로(실측), fail-closed는 **어댑터의 `verify_role_provisioned` 게이트**가 강제한다 — `AgentRunner.start_session`이 `backend.start` 전에 `artifact_root(workdir, env)` 기준으로 프로파일 존재·내용을 검사하고 없거나 다르면 기동을 거부한다. Phase 3는 이 명세를 **디스크에 물질화만** 하고(배치·쓰기·권한), 세션 env에 `CODEX_HOME`을 컨테이너 HOME 아래로 설정해 게이트 검사 위치와 `-p` 조회 위치를 일치시킨다 — 게이트는 `verify`가 받은 root의 파일을 검사하는 메커니즘일 뿐이라, 컨테이너 세션에선 호스트 파일과 컨테이너 파일이 다르므로 이 정렬(또는 검사 디렉터리를 컨테이너에 read-only bind mount)이 없으면 게이트가 Codex가 안 읽는 파일을 검사하게 된다(`FakeBackend`는 동일 호스트라 성립). 이 방식은 `up`(start_session)↔`attach`(assign/send) 프로세스 분리가 낳던 부트스트랩 타이밍 문제를 없앤다.
    - **미결 — Phase 3/슬라이스 B가 실측·배선한다(확정 아님).**
      1. **역할 프롬프트를 얹을 정확한 네이티브 표면.** `$CODEX_HOME`/전역 `AGENTS.md` 자동 로드는 그 내용이 **`user` 계층**에 들어가 workspace `AGENTS.md`가 의미적으로 override할 수 있다(Codex 0.144.3 `debug prompt-input` 관측) — 역할 정체성 계층(`developer`)이 아니다. 지운 런타임 주입(첫 타이핑 프롬프트)도 마찬가지로 `user` 계층이었다. 유력 후보는 프로파일의 `developer_instructions`(또는 검증 후 `model_instructions_file`) — 대상 Codex 버전에서 입력 계층·override·재시작 로드를 실측해 확정한다. 특히 프로파일 계층은 project config(`.codex/config.toml`)보다 우선순위가 낮아 신뢰된 작업본이 `developer_instructions`를 덮을 수 있다(공식 문서) — 그러면 `verify`의 파일 일치 검사만으로는 실효 역할 프롬프트를 보증하지 못하므로, override 차단 또는 우회(예: CLI 최고 우선순위 `-c developer_instructions=…` 주입)를 함께 실측·확정한다. 스펙 §CLI표(line 151)의 "본문 선두 주입" 전제는 이 결정으로 대체되므로 phase2 스펙 소유자가 갱신한다. **같은 갱신 범위(이 세션 스펙 정합성 리뷰에서 확인):** §2.3.3의 "역할 프롬프트를 exec 본문 선두에 cat + 래퍼가 `-s`·`-m`을 싣는" 조립 서술(→ `-p <role>` 프로파일이 프롬프트·sandbox·model을 얹음), §5 line 807의 "`prepare_subagents`가 workdir에 물질화"(→ 물질화 없이 빈 목록 반환, 물질화는 Phase 3·`$CODEX_HOME`), §5 모듈 계약·§9 Phase 5 행에 어댑터 계약 확장 `role_artifacts`·`verify_role_provisioned`·`artifact_root`(+`RoleArtifact`·`RoleNotProvisioned` 타입) 미기재.
      2. **Maintainer는 호스트다(이미지 아님).** Leader는 컨테이너 이미지에 굽지만 Maintainer는 호스트에서 도므로 "이미지 빌드 시 $CODEX_HOME 굽기"로 안 된다 — 호스트 물질화 경로를 별도로 배선한다. 또한 호스트 세션도 `start_session`에 `CODEX_HOME`을 명시 env로 넘겨야 한다 — 안 넘기면 게이트가 `~/.codex`로 폴백해, 세션이 상속하는 환경의 `CODEX_HOME`과 어긋날 수 있다.
      3. **보호.** tamper 방지는 `rule-protected-paths`가 아니라(그건 리포 push 경로 규칙이라 `$CODEX_HOME` 무관) 이미지·호스트의 **읽기전용 굽기 + OS 소유권/권한**으로 한다.
      4. **수용 게이트 + fail-closed 강제.** 기동 전 존재·내용 검사는 이제 **어댑터의 `verify_role_provisioned`가 소유**한다(`start_session`이 `backend.start` 전 호출; SESSION 역할). Phase 3가 이어받는 것은 ⓐ 세션 env에 `CODEX_HOME`을 설정해 게이트 검사 위치와 `-p` 조회 위치를 일치시키는 것, ⓑ 선택된 SUBAGENT 역할들의 프로비저닝 검증을 `leader.up`(prepare_subagents 물질화 시점)에서 호출하는 것, ⓒ 버전 고정 라이브 수용 테스트로 ① 역할별 프롬프트가 실제 `developer` 계층으로 로드됨 ② 프로파일 부재 시 기동 거부(음성 케이스)를 확정하는 것이다.
    - Claude는 `--append-system-prompt` argv(네이티브·workspace 밖·`developer`급)를 그대로 쓴다. agent_runner에는 런타임 주입 프리미티브를 두지 않는다 — 계층상 틀렸고(`user`) `up→attach` 취약을 낳기 때문이다.
  - `leader.up(platform)` 필수·`cli --platform`(기본 `claude-code`) — 스펙 §9 892~897행. `leader.up`이 `adapter.prepare_subagents`→`build_session_command`→`start_session(role, …)`을 실제로 배선한다(현재 `leader-placeholder.sh` 대체). `leader-placeholder.sh`의 `build_launch_command` 주석도 `build_session_command`로 고친다. (같은 `build_launch_command` 잔존이 `WIP/specs/2026-06-26-phase3-isolation-infra-design.md:104`에도 있어 Phase 3 스펙 소유자가 `build_session_command` 산출물로 갱신한다.)
  - Codex `prepare_subagents` — 어댑터의 이 메서드는 **빈 목록을 반환한다**(SUBAGENT는 세션 argv에 안 실림). 각 역할의 프로파일 명세는 `role_artifacts`가 낸다. Phase 3는 그 명세를 `$CODEX_HOME` 아래 물질화하고 `axdt-subagent` 래퍼를 이미지에 굽는다(§2.3.3). `.rules`의 역할별 의미·본문은 어댑터가 향후 `role_artifacts`로 계산(형식 슬라이스 B), Phase 3는 배치·쓰기만.
...
- ③ 역할별 실행 — `capability_args`·`prepare_subagents`(Claude `--agents`)·`build_session_command`(Claude는 `--append-system-prompt`, Codex는 `-p <role>` 프로파일 바인딩 — 역할 프롬프트는 프로파일이 얹음, §6), `build_launch_command` 대체, `start_session(role, …)`.
...
> 정본은 `PLATFORM_MATRIX`의 '순서·역행 정책 — 슬라이스 B 확정' 소절이다. 아래는 배경 사본이며, 확정 정책 반영도 그 소절에 한다(§3이 아니라).
...
4. **반영 위치.** `runner.py:poll_state`의 유보 주석과 매트릭스 '순서·역행 정책 — 슬라이스 B 확정' 소절을 확정 정책으로 교체하고, `PLATFORM_MATRIX`에 하트비트 주기·순서 키를 확정 행으로 추가한다.
