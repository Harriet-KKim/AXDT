...
- 기본 경로 `docs/sot/`. 인자로 경로를 주면 그 하위를 검사.
- 종류(디렉터리)로 파일을 분류: `requirements/`·`specification/`·`test-design/`·`rule/`.
- **전역 제외**: 파일명이 `README.md`이거나 `_TEMPLATE.md`로 끝나는 파일은 C1~C6 **전체에서 제외**. 단, `rule/` 파일의 `id` 수집(§5 rule 레지스트리)에서도 `rule/_TEMPLATE.md`는 제외한다.
...
- **오류 처리(종료코드 2)**: frontmatter 부재, YAML malformed, 중복 키는 위반이 아니라 **실행 오류**로 처리한다(검사 불능).
...
### 3.2 본문 항목 표기
- 본문 항목 ID는 **볼드 표기**(`**FR-1**`, `**SP-3**`, `**TD-2**`)로 식별한다.
- 본문 볼드는 **선언 자리(정의)와 참조 자리(수용 기준 등)에 여러 번 등장**하는 게 정상이다. 선언 유일성은 본문이 아니라 frontmatter `items`에서 검사한다(§4 C2).

### 3.3 코드 펜스·인라인 코드 상태머신
- C4(플레이스홀더)·C5(금지어)는 **코드 블록(``` 펜스)과 인라인 코드(`백틱`) 안을 검사에서 제외**한다.
- 라인 단위 스캔에 펜스 상태(펜스 진입/이탈)를 유지하고, 라인 안에서는 인라인 백틱 구간을 마스킹한 뒤 검사한다.

## 4. 검사 항목 (C1~C6)

선언 정본은 frontmatter `items`다. 존재·항목 ID·참조 무결성은 frontmatter를 대조해 결정적으로 판정한다. 본문 산문은 두 곳에서만 읽는다: (a) `items`↔본문 볼드 정합, (b) C6 수용 기준 체크박스.
...
- **C2 항목 ID + 유일성** — 각 문서 `items`의 항목 ID가 종류 규약을 따른다: 요구 `FR-<정수>`·`NFR-<정수>`, 사양 `SP-<정수>`, 테스트 설계 `TD-<정수>`.
  - 규약을 벗어난 ID는 위반.
  - **유일성**: 같은 `(topic, ID)` 조합이 SoT 전체에서 둘 이상 `items`에 선언되면 위반(중복 선언은 참조 대상을 모호하게 만든다). topic=파일 stem이라 한 문서 내 `items` 중복이 곧바로 걸린다.
  - **본문 정합(a)**: `items`에 선언된 각 ID가 본문에 볼드로 최소 1회 등장해야 하고, 본문에 볼드로 나온 항목 ID(종류 규약에 맞는 것)는 `items`에 선언돼 있어야 한다. 어긋나면 위반(선언에 없는 본문 항목, 본문에 없는 선언 항목 모두).
  - **id↔topic**: frontmatter `id`가 `<접두>-<topic>` 규약(요구 `req-`, 사양 `spec-`, 테스트 설계 `td-`)과 파일 stem에 일치해야 한다. 불일치 위반.
- **C3 참조 무결성** — `covers`·`rules`가 가리키는 대상이 실재(dangling 없음). frontmatter↔frontmatter 대조.
  ...
  - **교차 topic 접두**: `covers` 원소가 `<topic>:FR-1` 형태면 그 topic 문서의 items에서 찾는다. 접두가 없으면 **참조하는 문서와 같은 topic**의 문서에서 찾는다.
  - **rules → rule 레지스트리**: `*.rules`의 각 원소가 실재 rule id여야 한다. 레지스트리 = `docs/sot/rule/*.md`(단 `_TEMPLATE.md` 제외) 각 파일 frontmatter `id`의 집합.
  - `covers`에 미치환 `<...>`가 남아 있으면 항목으로 해소하지 않고 **C4가 잡는다**(참조 실패가 아니라 플레이스홀더 위반).
  - `related`는 검사 제외.
- **C4 플레이스홀더 없음** — 미치환 `<...>` 없음. 코드 블록·인라인 코드 제외(§3.3).
  - 정규식은 산문의 부등호 오검을 피하도록 특이하게: `<`와 `>` 사이에 개행이 없고 공백으로 시작하지 않는 토큰(예: `<판정 기준.>`은 위반, `a < b`·`x<y`는 비위반)만 잡는다. 세부 정규식은 구현 시 테스트로 고정.
- **C5 금지어 없음**(결정A) — 닫힌 최소 목록만 검사, 코드 상수로 단일 관리:
  - 목록: `TBD`, `TODO`, `FIXME`, `미정`, `추후`, `나중에`.
  - ASCII 토큰(`TBD`·`TODO`·`FIXME`)은 대소문자 무시 + 단어 경계 매칭(다른 단어의 일부는 제외). 한글(`미정`·`추후`·`나중에`)은 정확 매칭.
  - 코드 블록·인라인 코드 제외(§3.3). 오검이 나오면 후속 조정.
- **C6 수용 기준 자리 채워짐** — 각 요구·사양·테스트 설계 항목의 수용 기준 자리에 플레이스홀더·금지어가 아닌 문장이 있음.
  - 수용 기준 항목은 `- [ ] **<ID>** <내용>` 체크박스로 식별한다.
  - 그 `<ID>`가 `items`에 선언된 항목이어야 하고(그렇지 않으면 C2 정합에서 이미 잡힘), `<내용>`이 비어 있거나 플레이스홀더(`<...>`)·금지어면 위반.
  - **검증 가능한지·무의미한 filler인지는 판정하지 않는다** — ②의 몫.
- **범위 밖(안 한다)** — 커버리지 완결성(모든 요구가 사양·테스트로 커버되는지, 고아 항목 등)은 ②의 몫이라 검사하지 않는다.

## 5. rule id 레지스트리

- `docs/sot/rule/*.md` 각 파일의 frontmatter `id`를 수집해 집합을 만든다. `rule/_TEMPLATE.md`·`rule/README.md`는 제외.
...
- 옵션: `--json`.
- **종료코드**: clean=0, 위반 있음=1, 실행 오류=2.
- **텍스트 출력**: 위반마다 한 줄 `파일:라인 · 코드(C1~C6) · 메시지`. 위치가 파일 전체/디렉터리 수준이면 라인 생략.
- **`--json` 스키마**(고정):
  ```json
  {
    "schema_version": 1,
    ...
    "summary": {"files": 0, "violations": 0, "by_code": {}}
  }
  ```
  - `line`은 위치가 특정 라인이 아니면 `null`(C1 종류 부재 등).
  - `ok` = `violations`가 비었는가.

## 7. 결정성

- 같은 입력 = 같은 출력(정렬 포함).
- **정렬 키**: `(path, line, code, message)`를 유니코드 코드포인트로 정렬(locale 무관). `line`이 null이면 정렬상 가장 앞(예: -1 취급).
...
- C2의 세부 위반 코드 분리(형식 위반 vs 유일성 위반 vs 본문 정합 위반)를 하나로 묶을지 나눌지는 구현 시 결정. 메시지로 구분 가능하면 단일 코드로 둔다.
...
- 구현 산출물: `WIP/axdt/sot_lint/`(패키지 `__init__.py`·`__main__.py`·검사 로직) + `WIP/axdt/sot_lint/tests/` 또는 프로젝트 pytest 규약에 맞춘 위치.
