...
| `config.py` | `.axdt/`·`worktrees/` 경로, 포트 파생, 전송 선택 |
| `hub.py` | 로컬 bare 허브(권위 상태) init/serve, clone URL(호스트/컨테이너) |
| `worktree.py` | 허브 clone 기반 격리 작업본 provision/teardown |
...
**branch·worktree dir·container를 모두** 구동한다.
- branch = `w3.t12-auth-login`
- worktree dir = `worktrees/w3.t12-auth-login`
...
worktrees/<id>/         # Leader별 격리 클론 (컨테이너 RW 마운트 유일 경로)
```
