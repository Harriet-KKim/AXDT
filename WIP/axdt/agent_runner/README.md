...
- 역할 정체성: Claude는 `--append-system-prompt` argv(네이티브·workspace 밖)로 전달한다. Codex 세션 역할 프롬프트는 런타임 주입이 아니라 Phase 3가 `$CODEX_HOME` 아래 네이티브 파일(workspace 밖·읽기전용/protected)로 물질화해 Codex가 네이티브 로드한다(handoff §6). 역할 텍스트 SoT는 `roles/prompts/<role>.md`.
