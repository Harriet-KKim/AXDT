...
class ParseGlossaryTest(unittest.TestCase):
    ...
    def test_ignores_wide_non_glossary_table(self):
        # 4열 파일역할 표 같은 비-용어집 표는 오파싱하지 않는다(2열만 채택)
        text = ("| 파일 | 역할 | 작성자 | status |\n"
                "|---|---|---|---|\n"
                "| report | task별 상세 | Leader | report.status |\n"
                "| progress | 색인 | Maintainer | progress.status |\n")
        self.assertEqual(build.parse_glossary(text), {})
...
def parse_glossary(text: str) -> dict:
    ...
    for line in text.splitlines():
        ...
        if m:
            ...
            # 용어집 표는 정확히 2열(용어|풀이)일 때만 채택한다. terminology.md의
            # 4열 파일역할 표(| 파일 | 역할 | 작성자 | status |)가 term→def로
            # 오파싱되는 것을 막는다(재리뷰 blocking).
            if len(cells) != 2:
                continue
...
> **재리뷰 반영(이 태스크에서 구현)**: 카드는 **이전 라운드 의견만** 보여야 한다(§2.8). `render_comment_cards(comments, anchor, max_round=None)`·`inject_comment_cards(body, comments, max_round=None)`·`_CardInjector(comments, max_round)`에 `max_round`를 더하고, `rnd["round"] < max_round`인 라운드만 렌더한다. `build_page`는 `meta["round"]`를 넘긴다. 통합/카드 테스트는 `meta.round=2`·comment `round=1`로 두어 "현재 라운드(2) 의견은 카드로 안 뜨고 이전 라운드(1)만 뜬다"를 검증한다(기본값 `None`이면 전량 렌더 — 단위 테스트 호환).
...
class IngestTest(unittest.TestCase):
    ...
    def test_ingest_replaces_not_appends_within_round(self):
        # 같은 라운드에서 두 번 회수해도 중복되지 않는다(교체 병합)
        with tempfile.TemporaryDirectory() as root:
            slug_dir = self._fixture(root, round_=2)
            clip = Path(root) / "clip.json"
            clip.write_text(build.serialize_comments_json(
                "sot-gate", 2, [{"anchor": "SP-3", "kind": "수정요청", "text": "a"}]),
                encoding="utf-8")
            build.ingest(slug_dir, clip)
            clip.write_text(build.serialize_comments_json(
                "sot-gate", 2, [{"anchor": "SP-3", "kind": "수정요청", "text": "a"},
                                {"anchor": "SP-5", "kind": "질문", "text": "b"}]),
                encoding="utf-8")
            comments = build.ingest(slug_dir, clip)
            self.assertEqual(len(comments["rounds"][0]["items"]), 2)
...
def merge_comments(comments: dict, rnd: int, items: list) -> dict:
    ...
    for entry in rounds:
        if entry.get("round") == rnd:
            # 클립보드는 그 라운드의 items 전체를 담으므로 교체한다(append 아님).
            # append면 라운드 내 재회수 시 앞 항목이 통째로 중복된다(재리뷰 blocking).
...
> **재리뷰 반영(이 태스크에서 구현)**: `<script>`는 세션·슬러그·라운드 값을 **JS 문자열 리터럴로 직접 주입하지 않는다**(`var SESSION="{{SESSION}}"` 금지). `build_page`가 HTML-escape한 값을 JS에 넣으면 `A & B`→`A &amp; B`로 오염되고, meta에 개행이 있으면 JS 구문이 깨진다(재리뷰 지적). 대신 루트 요소에 `data-session`·`data-slug`·`data-round`로 담고(HTML 속성 문맥이라 build_page의 단일 escape로 안전), JS는 `document.getElementById('rr-root').dataset`에서 읽는다. 즉 `{{SESSION}}`·`{{SLUG}}`·`{{ROUND}}` 토큰은 **HTML 속성 안에만** 놓는다. shell의 JS `KINDS` 배열은 Task 6 `build.KINDS`와 값이 일치해야 하고, 이를 `test_kinds_match_python_constants`로 검증한다.
...
def render_shell(shell_html: str, values: dict) -> str:
    ...
    # _sub가 shell_html을 1회 스캔하며 알려진 토큰을 치환하고, 값이 없으면 즉시 raise한다.
    # 따라서 치환 후 남는 {{KNOWN}}은 삽입된 값(BODY·CONTEXT 등) 안의 리터럴뿐이다.
    # 이를 미치환으로 오판하면 이 스킬로 자기 템플릿 문서(토큰 예시 포함)를 리뷰할 때
    # 오탐 빌드 실패가 난다(재리뷰 지적). shell.html에 토큰이 빠진 경우는
    # Task 8 test_all_known_tokens_present가 잡으므로 여기서 leftover 검사는 하지 않는다.
    return _TOKEN_RE.sub(_sub, shell_html)
...
```
`glossary_sources`는 **빈 배열로 시작**한다. `docs/sot/rule/terminology.md`는 용어집이 아니라 규칙 문서다(정의가 `=` 등호 형식이라 파서 미지원 + 4열 파일역할 표가 `report`·`progress`·`파일`을 틀린 툴팁으로 오파싱). Backlog 373의 진짜 용어집(`- **용어** — 풀이` 또는 2열 표)이 생기면 그 경로를 추가한다. 그 전까지 자동 감싸기는 없고 수동 `<b-term>`이 실효 경로다(설계 §4.1).
...
glossary_sources가 비어 있어 자동 감싸기는 없고, 수동 b-term만 툴팁으로 뜬다.</p>
</section>
EOF
...
```
Expected 출력에 `섹션 1개`가 있어야 한다. `glossary_sources`가 비어 있으므로 자동 감쌈은 0이고(정상), 용어 풀이는 수동 `<b-term>`으로만 검증한다.
