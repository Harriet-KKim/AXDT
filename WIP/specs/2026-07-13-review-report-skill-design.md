...
```
경로는 프로젝트 루트 기준. `glossary_sources`는 순서대로 병합, 겹치면 뒤 소스가 이긴다. **지정한 소스 파일이 없으면 빌드 실패**(빈 배열은 소스 0개라 실패 아님). 입력은 `utf-8-sig`로 읽어 BOM을 허용한다.

**지금은 빈 배열로 시작한다.** `docs/sot/rule/terminology.md`는 용어집이 아니라 규칙 문서다 — 정의는 `- **SoT** = …` 등호 형식(파서 미지원)이고, 4열 파일역할 표(`| 파일 | 역할 | 작성자 | status |`)가 있어 소스로 걸면 `report`·`progress`·`파일` 같은 흔한 단어가 틀린 툴팁으로 오파싱된다. Backlog 373의 진짜 용어집(`- **용어** — 풀이` 또는 2열 표)이 생기면 그 경로를 추가한다. 그 전까지 자동 감싸기는 없고 **수동 `<b-term>`이 유일한 실효 경로**다(용어 풀이 기능 자체는 수동으로 동작). 파서는 방어적으로 2열 표만 채택해 미래의 다열 표 오파싱도 막는다.
