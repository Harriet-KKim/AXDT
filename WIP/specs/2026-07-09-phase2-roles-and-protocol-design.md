...
> 상태: **10차 개정본 — 3모델 재리뷰 만장일치 치명 0으로 수렴** (문구 정밀화 반영) · 초안 2026-07-09 · 2~5차 개정 2026-07-10 · 6~10차 개정 2026-07-12 · 범위: Phase 2 (WIP/TODO.md) · 다음: 슬라이스 B(상태 파일 신선도 정책·라이브 측정) → §8.3a 잔여 항목(§8.3a 주 재-시퀀싱; 상태판정 재설계는 ADR-0016으로 착지)
...
> 관련 ADR: `WIP/adr/0001`(상시 tmux Maintainer), `0002`(무 DB/큐/이벤트로그), `0003`(tmux 하향·report 상향·Leader 허브), `0004`(report→progress 권위 흐름), `0005`(agent runner 합성·주입 backend), `0006`(허브 git 격리), `0007`(계층 강제), `0016`(상태판정=훅 기반 상태 파일·`ERROR`/`STOPPED`는 프로세스 생존으로 판정), `0017`(Codex 역할=`-p <role>` 프로파일 바인딩·어댑터 계약 보증)
> **정합화 갱신(2026-07-22)**: Phase 5 구현이 확정한 ADR-0016(상태판정)·ADR-0017(Codex 역할 전달)에 맞춰 §2.3.3·§2.3 CLI표·§2.4·§2.5·§4.1·§4.5·§5 어댑터 계약·§7.1·§7.3·§8.1·§8.3a·§9·§11의 관련 서술을 정합화했다. 인계: `WIP/handoff-phase5-runtime-contract.md` §6.
...
| 시스템 프롬프트 | `--append-system-prompt` | **전용 플래그 없음** — 역할은 `-p <role>` 프로파일이 얹는다(ADR-0017), 본문 선두 주입 아님 | `-p` 바인딩·게이트 실측/확정 / 실효 프롬프트 표면·계층은 슬라이스 B 잠정 |
...
/usr/local/bin/axdt-subagent <role> "<지시>"
  └ AXDT가 조립:  codex exec -p <role> "<지시>"
```

역할 프롬프트·샌드박스 등급·모델은 **`-p <role>` 프로파일이 얹는다**(ADR-0017). 옛 스케치처럼 역할 파일을 `cat`으로 본문 선두에 붙이거나 `-s`·`-m`을 래퍼가 따로 싣지 않는다 — 프로파일 하나가 역할·권한을 담고, 지시만 위치 인자로 넘긴다. 프로파일 명세를 **계산·보증하는 단일 진실원은 어댑터**다: `role_artifacts`가 프로파일 내용을 내고, `verify_role_provisioned`가 기동 전 fail-closed로 존재·내용을 검사한다(§5·ADR-0017).

**프로파일 위치는 `$CODEX_HOME` 아래이지 workspace가 아니다.** `-p <profile>`은 `$CODEX_HOME/<name>.config.toml`을 겹쳐 얹으므로(실측), 프로파일을 workspace에 두면 찾지 못한다. 컨테이너에서 `$CODEX_HOME`은 `HOME`(`/tmp/axdt-home`) 아래다. **어댑터는 이 명세를 계산만 하고, 디스크 물질화(배치·쓰기·권한)는 Phase 3가 `artifact_root` 아래 수행한다**(ADR-0017 — 물질화 시점은 Phase 3 몫이고, §5 `prepare_subagents`는 물질화 없이 빈 목록을 반환한다).

workspace에 두지 않는 두 번째 이유가 있다. workspace는 git clone이므로, 역할 파일을 거기 쓰면 작업 트리를 오염시키고 Leader가 실수로 커밋할 수 있다.

**명령줄을 조립하는 주체가 누구인가가 강제 등급을 가른다.** Leader LLM이 직접 `codex exec -s read-only ...`를 타이핑한다면 그것은 권고 등급이다 — 옵션을 빼먹으면 그만이다. 래퍼 스크립트를 심고 Leader에게 `Bash(axdt-subagent *)`만 허용하면, 명령줄의 소유권이 AXDT로 넘어와 **게이트 등급**이 된다. 여기에 더해 `verify_role_provisioned` 게이트가 프로파일 부재·불일치 시 기동을 거부해 fail-closed를 보강한다(ADR-0017). 후자를 택한다.
...
2. **`ERROR`는 프로세스 죽음에서만 나온다 — 살아 있는 세션은 `ERROR`가 아니다(ADR-0016).** 화면 마커 판정은 폐기됐다: `poll_state()`는 훅이 쓴 상태 파일로 `idle`·`busy`·`waiting_input`·`starting`을 읽고, `ERROR`·`STOPPED`는 상태 파일이 아니라 프로세스 생존(`is_alive`·`exit_code`·`last_error`)으로만 판정한다. 따라서 재기동은 프로세스의 실제 죽음을 관측했을 때만 하며(하한선 1), 옛 개정본이 우려한 "테스트 실패 한 줄이 멀쩡한 세션을 `ERROR`로 오판" 경로 자체가 사라졌다.
...
조사 중 정정된 오판 하나가 후보 흐름을 연다. 초안은 "대화형 명령이라 외부 프로세스가 결과를 받을 수 없다"고 적었는데, 그것은 일반적인 CLI를 가정한 오판이었다. **`send-keys`로 문자를 밀어넣고 캡처 로그로 화면을 읽는 경로는 실재한다** — 이미 그 경로로 문자 주입·화면 캡처를 수행한다(실측). (상태판정 자체는 ADR-0016 이후 화면이 아니라 훅 상태 파일로 한다 — 이 경로는 `/context` 사용량 파싱 같은 화면 열람에 쓴다.)
...
캡처 로그 오프셋은 파일 끝으로 두되, 로그 열람용으로 마지막 `TAIL_WINDOW` 바이트를 읽어 transcript를 시드한다. **상태판정은 이 transcript가 아니라 훅 상태 파일로 한다(ADR-0016).** attach 직후의 첫 판정은 상태 파일 신선도 정책이 미확정이라(하트비트·`ts` 임계는 슬라이스 B) 신뢰하지 않고, 주입 전에 `wait_until_idle(timeout)`으로 재폴링한다. 재폴링은 `BUSY`·`STARTING`이 `IDLE`로 가라앉기를 기다릴 뿐, 첫 판정이 이미 `IDLE`이면 즉시 반환하므로(실측: `runner.py`) 신선도를 보증하지는 않는다 — 신선도 보증은 슬라이스 B의 `ts` 임계가 들어와야 성립한다.
...
**행 4(ERROR)는 `poll_state`의 실제 의미론에 정직하다.** ADR-0016으로 화면 마커 판정이 폐기돼, `poll_state`의 `ERROR`는 이제 **죽은 세션에서만** 나온다(실측: `runner.py`가 `not is_alive()`에서 `last_error()`가 있거나 `exit_code()`가 비영이면 `ERROR`, 깨끗이 죽으면 `STOPPED`; 살아 있는 세션은 훅 상태 파일로 `idle`·`busy` 등으로 읽힌다). `ERROR`가 곧 죽음을 뜻하므로 §7.3은 이 표를 부르기 전에 `is_alive()`로 재확인해 재기동 경로로 보낸다(§7.2). 살아 있는 세션이 행 4에 오는 경로는 사라졌다. 표를 단독으로 쓰는 호출자(Phase 8 등)에게 행 4는 안전한 backstop이다 — 죽은 세션을 블로커로 보내는 것은 조용히 무시하는 것보다 낫고, 재기동이 필요하면 그 호출자도 §7.3처럼 `is_alive`를 먼저 확인해야 한다. **행 3(STOPPED→`None`)은 이 backstop이 비대칭임을 드러낸다** — 행 4가 ERROR를 블로커로 올리는 것과 달리 STOPPED(정상 종료 사망)는 `None`(정상, 할 일 없음)이라, `is_alive` 관문 없이 표만 쓰는 소비자는 **깨끗이 죽은 Leader를 아무 신호 없이 방치**한다. §7.3 경로에서는 관문이 STOPPED를 표 앞에서 재기동으로 보내 문제없으나, Phase 8 계약은 "ERROR뿐 아니라 STOPPED도 `is_alive` 선행"을 명시해야 한다(§9로 넘기는 요구).
...
**주입자 크래시로 인한 유실이 사라진다.** 주입자가 어느 지점에서 죽든, 다음 주기에 같은 관측이 같은 지시를 다시 만든다. `SKIPPED`라는 결과가 없으므로 `SKIPPED`가 삼키던 지시도 없다. 이것은 *배달 기록을 두지 않기 때문에* 성립하며, 상태 판정의 정확도에 기대지 않는다.

**중복 실행은 `IDLE` 게이트가 줄인다.** 지시가 실제로 제출됐다면 Leader는 `IDLE`을 벗어나고, 그 상태에서는 아무것도 보내지 않는다. 다만 게이트의 신뢰도는 훅 상태 파일 판정의 신뢰도이고 그것은 아직 잠정이므로(상태 파일 신선도 정책이 슬라이스 B 미확정), **"막는다"가 아니라 "줄인다"**로 적는다. §8.3이 확정하기 전까지 이 문장은 조건부다.
...
**토큰은 남기되 오라클이 아니다.** 메시지 앞머리의 `[axdt:assign:<task>:<hash8>]`는 사람이 로그를 읽을 때의 표식이고 테스트가 렌더링을 고정하는 수단이다. **어떤 분기도 토큰의 로그 존재 여부를 보지 않는다.** 따라서 캡처 로그는 원래 역할(로그 열람용 transcript)로 돌아가고, 보존 의무도 적대적 쓰기 표면도 없다.
...
**주입은 기존 `AgentRunner.send_prompt()`를 그대로 쓰지 않는다.** 그 함수는 원래 `IDLE`·`WAITING_INPUT`을 둘 다 받았으나(옛 `INPUT_ACCEPTING`), Phase 5 슬라이스 A가 `IDLE` 단독으로 좁혔다(실측: `runner.py`, `INPUT_ACCEPTING` 제거). 주입은 그와 별개로 `clear_input`을 포함한 `IDLE` 전용 경로(`send_when_idle`)를 쓴다.
...
**자유 텍스트(`note`)가 상태를 오염시키던 경로는 닫혔다.** 옛 개정본은 `note`가 `Error:` 같은 마커 문자열을 품으면 `detect_state`가 화면에서 그것을 읽어 상태 판정이 오염된다고 경계했다. 상태판정이 훅 상태 파일로 바뀌어(ADR-0016) `detect_state`가 화면 문자열이 아니라 상태 파일 값을 매핑하므로(실측: `adapters/base.py`), 이 오염 경로와 그에 걸린 제약은 사라졌다.
...
멱등을 상태 수렴으로 옮겼으므로 이 구분이 더 이상 유실을 만들지는 않는다. 그러나 **오염은 만든다.** 세션의 입력창은 하나뿐인 공유 자원이고, 타이핑은 훅을 발화시키지 않아 상태 파일이 `idle`에 머무르므로 타이핑 중인 세션도 `IDLE`로 보인다.
...
- 그래도 창은 남는다. 재폴링과 삭제 키 사이에서 세션이 스스로 전이할 수 있고, 락은 다른 주입자를 배제할 뿐 **에이전트 자신의 전이를 막지 못한다.** 이것을 수용된 잔여 위험으로 명시한다. (상태 파일 신선도 정책 자체는 슬라이스 B에서 확정한다.)
...
이 오라클은 **"`IDLE`인 에이전트는 입력 없이 `IDLE`을 벗어나지 않는다"** 는 미측정 전제 위에 있고, 훅 상태 파일 판정도 아직 잠정이다(신선도 정책이 슬라이스 B 미확정). 그래서 §8.3의 측정 항목이다.
...
**재기동은 프로세스의 실제 죽음을 확인한 뒤에만 한다.** ADR-0016으로 화면 마커 판정이 폐기돼, `poll_state()`의 `ERROR`는 이제 죽은 세션(비영 종료 코드거나 `last_error` 표면화)에서만 나온다 — 살아 있는 Leader는 훅 상태 파일로 `idle`·`busy` 등으로 읽히지 `ERROR`가 아니다. 옛 개정본이 우려한 "테스트 실패 한 줄이 멀쩡한 Leader를 `ERROR`로 오판" 경로는 사라졌다.

재기동 전에 **프로세스의 실제 죽음을 확인한다** — `is_alive()`가 거짓이거나, `attach()`가 실패하고 `post_mortem()`이 종료 코드를 돌려줄 때만. `ERROR`가 곧 죽음을 뜻하므로(ADR-0016) 이 확인이 재기동의 유일 관문이다. §2.4가 Watcher에 건 하한선과 같은 규칙이며, 주입·수렴·본 절의 모든 경로에 적용된다.
...
# 임포트: Mapping·Sequence(collections.abc), Path(pathlib), dataclass, ABC·abstractmethod

@dataclass(frozen=True)
class RoleArtifact:
    """기동 전 물질화될 아티팩트 하나. 정본은 `adapters/base.py`."""
    path: Path        # artifact_root 기준 상대경로 (절대·`..` 이탈은 __post_init__이 거부)
    content: str      # 역할 프롬프트·권한을 담은 본문
    mode: int = 0o400 # 권한 힌트(읽기전용). 실제 OS 권한 적용은 Phase 3.

class RoleNotProvisioned(Exception):
    """역할 아티팩트가 없거나 어댑터 계산본과 내용이 다르다(fail-closed 거부)."""

class PlatformAdapter(ABC):
    # 기존 구체: config_dir, format_prompt, submit_key, clear_key, detect_state
    @abstractmethod
    ...
    def prepare_subagents(self, workdir: Path, roles: Sequence[RoleSpec]) -> list[str]:
        ...
        Codex: 물질화 없이 빈 목록 반환 — 프로파일 명세는 `role_artifacts`가 내고,
               디스크 물질화는 Phase 3가 `$CODEX_HOME` 아래 수행한다(ADR-0017).
    ...
    def build_session_command(self, role: RoleSpec, workdir: Path,
        ...
        """SESSION 역할의 실행 argv — capability·model·subagents 조립. 역할 프롬프트 전달은 플랫폼별이다:
        Claude는 `--append-system-prompt` argv로 싣고, Codex는 `-p <role>` 프로파일이 고르는 외부 아티팩트로
        전달한다(argv에 직접 싣지 않음, ADR-0017)."""

    @abstractmethod
    def role_artifacts(self, role: RoleSpec, root: Path) -> list[RoleArtifact]:
        """역할 프롬프트·권한(sandbox 등급)을 담은 아티팩트 명세를 계산한다 — 역할·권한 내용의 단일 진실원(ADR-0017).
        Claude는 argv·--agents로 자립하므로 빈 목록("보증 없음"이 아니라 "검증할 외부 아티팩트 없음").
        Codex는 `-p <role>` 프로파일(`<role>.config.toml`) 명세를 낸다. 디스크 물질화는 Phase 3."""

    # 아래 둘은 구체(공통 기본 구현)다 — 어댑터가 role_artifacts만 채우면 게이트가 성립한다.
    def artifact_root(self, workdir: Path, env: Mapping[str, str]) -> Path:
        """게이트·물질화가 합의하는 아티팩트 위치. 기본값은 workspace의 `config_dir`.
        Codex는 env `CODEX_HOME`(없으면 `~/.codex`)를 읽으므로 이 메서드를 override한다(ADR-0017)."""

    def verify_role_provisioned(self, role: RoleSpec, root: Path) -> None:
        """기동 전 fail-closed 게이트 — `role_artifacts` 명세와 `root` 아래 실제 파일을 대조하고,
        부재·내용 불일치·읽기 실패면 `RoleNotProvisioned`를 던진다(빈 목록이면 통과, 권한 검증은 Phase 3 몫).
        `AgentRunner.start_session`이 SESSION 역할에 대해 `backend.start` 전에 호출한다(ADR-0017)."""
...
**`format_prompt`와 `send_prompt`를 함께 옮긴다.** `format_prompt`가 제출 개행을 떼면(docstring "including the submit newline" 삭제) `send_prompt`가 제출을 잃으므로, `send_prompt`도 `submit()`을 부르게 고친다 — Phase 5 슬라이스 A가 이를 반영했다(실측: `runner.py`의 `send_prompt`가 `send_text` 후 `submit()`을 부른다).

**그러면서 `send_prompt`의 게이트를 `IDLE` 단독으로 좁힌다 — Phase 5 슬라이스 A가 반영했다.** 옛 `send_prompt`는 `IDLE`·`WAITING_INPUT`을 둘 다 받았는데(옛 `INPUT_ACCEPTING`), 제출을 붙이는 순간 **`WAITING_INPUT`에 걸린 세션에 Enter를 눌러 권한 프롬프트를 승인하게 되기 때문이다.** §4.1이 `clear_input` 때문에 세 문단을 들여 막은 바로 그 사고를, 이 함수가 뒷문으로 다시 연다. 권한 프롬프트에 답하는 것은 별도 동작이어야 하며 본 Phase에서 만들지 않는다(Phase 7).
...
class AgentRunner:
    ...
    def send_when_idle(self, text: str) -> bool:
        ...
        주입은 clear_input이 필요해 이 전용 경로를 쓴다
        (send_prompt도 슬라이스 A로 IDLE 단독이 됐으나 clear_input은 하지 않는다)."""
```
...
| `PlatformAdapter`에 추상 4종 추가(능력·subagent·세션 3종 + 역할 아티팩트 1종) + 구체 2종(`artifact_root`·`verify_role_provisioned`, ADR-0017), `build_launch_command` 대체 | `adapters/claude_code.py`, `adapters/codex.py`, **`test_adapters.py`의 `BareAdapter`** — `build_launch_command`만 구현하므로 인스턴스화 불가가 된다 |
...
| `leader.up()`에 `platform` 인자 추가(기본 `claude-code`)·`PLACEHOLDER` 제거 | `infra/tests/test_leader.py` **6개 중 5개** — `PLACEHOLDER` 명령 기대가 깨지고, `env` fixture가 어댑터·`ROLES`를 모킹하지 않아 실제 세션 조립에서 실패한다 |
| `cli.py`의 `leader up`에 `--platform` | `infra/tests/test_cli.py` — `--platform` 없이 호출한다 |
| `send_prompt`의 게이트를 `IDLE` 단독으로 좁힘 + `submit()` 호출 (Phase 5 슬라이스 A 반영) | `agent_runner/tests/test_runner.py` — 옛 `test_send_prompt_accepted_in_idle_and_waiting_input`가 `..._and_rejected_in_waiting_input`으로 갱신됨(실측: `WAITING_INPUT` 거부를 기대), 제출이 `submit()`으로 분리됨 |
...
def up(root, i, *, platform: str = "claude-code", reuse_workspace: bool = False, ...) -> TmuxDockerBackend:
    ...
    PLACEHOLDER를 제거하고, 세션은 AgentRunner.start_session으로 기동한다 —
    build_session_command·backend.start를 up()이 직접 부르지 않는다(그 경로는 게이트를 우회한다):
        sub_args = adapter.prepare_subagents(cwd, SUBAGENT_ROLES)
        art_root = adapter.artifact_root(cwd, env)
        for r in <선택된 SUBAGENT_ROLES>:            # SUBAGENT 프로비저닝 검증(ADR-0017)
            adapter.verify_role_provisioned(r, art_root)
        AgentRunner(adapter, be).start_session(ROLES["leader"], cwd, env, sub_args)
        # start_session이 내부에서: SESSION(leader) verify_role_provisioned → build_session_command → backend.start
    platform 인자가 필요한 이유는 어댑터를 고르기 위해서다(D4). 현재 up()에는 없다.

    기동 전 fail-closed 게이트(ADR-0017): SESSION(leader)은 start_session이, 선택한 SUBAGENT는
    up()이 backend.start 전에 verify_role_provisioned로 검증한다 — 부재·불일치면 RoleNotProvisioned로 기동을 거부한다."""
...
def restart(root, i, *, platform: str = "claude-code") -> TmuxDockerBackend:
    ...
    호출 전에 프로세스의 실제 죽음을 확인해야 한다 — `ERROR`는 죽음에서만 나오므로(ADR-0016) `is_alive`·`post_mortem` 확인이 관문이다(§4.5)."""
```
...
2. `leader.up(root, i, platform=...)` — workspace provision → `prepare_subagents` → 역할 프로비저닝 검증(`verify_role_provisioned`: SUBAGENT는 `up`, SESSION은 `start_session`, ADR-0017) → 컨테이너·tmux 기동.
...
3. `st = runner.wait_until_idle(timeout)` — attach 직후의 첫 `poll_state()`는 상태 파일 신선도 정책이 미확정이라 신뢰하지 않는다(§2.5, 슬라이스 B). `IDLE`이 아닌 채 시간이 다하면 상태를 가른다:
   - `ERROR`·`STOPPED` → `is_alive()`로 죽음을 확인한다(§4.5). ADR-0016으로 이 둘은 프로세스 죽음에서만 나오므로, 확인되면 §7.2 재기동으로 보낸다. 이 확인이 결정표에 `ERROR`를 넘기기 전의 관문이다.
...
초안은 이것을 "세로축 한 바퀴 검증"이라 불렀다. **과장이었다.** 가짜 백엔드 위에서 "Leader가 report를 쓴다"는 부분은 테스트 코드가 직접 파일을 써 넣는 것이므로, 확인되는 것은 모듈 경계의 호출 순서·타입·거부 범위뿐이다. 실제 전달 의미론과 상태 파일 계약의 라이브 정확도는 이 층이 잡지 못한다.
...
- **`needed_instruction()`의 전함수성과 상호배타성** — 모든 (plan × progress 9종 × report {부재,무효,5종} × rework × session 6종) 조합에 정의된 결과가 있고, 고정 평가 순서에서 첫 일치가 유일하다(각 행 조건이 앞 행 배제 하에 정의역을 좁힌다). **`progress` 분할 네 구간(종결·보류/반려/행없음/착수=행 14~17) 사이는 재정렬해도 결과가 같음**을 고정하되, **두 행은 순서 의존임**을 함께 고정한다: (a) **무효 행(행 7)** — `report`축 조건이라 종결·보류 뒤, 그리고 `report` 열이 `—`인 모든 구간(반려·행없음)과 착수 앞이 아닌 곳으로 옮기면 결과가 바뀐다. 반례 셋: `rejected`·무효를 반려 뒤로 옮기면 반려 발송, `accepted`·무효를 종결·보류 앞으로 옮기면 블로커, **`(plan=없음, 행없음, 무효)`를 행없음 뒤로 옮기면 행 13이 삼켜 `None`(고아 깨진 report 무신호 방치)**. (b) **catch-all 행(행 18)** — 전정의역을 받으므로 맨 마지막이 아니면 다른 행을 삼킨다(반례: 착수 블록과 함께 반려 앞으로 옮기면 `rejected`·`done`·`거짓`이 행 11 대신 행 18에 걸려 블로커가 된다). 즉 "행 7·18을 뺀 네 구간 재정렬 불변" + "행 7 위치 고정(종결·보류 뒤, 반려·행없음·착수 앞)" + "행 18 최후 고정"의 세 성질로 나눠 검증한다(재정렬 불변을 행 7이나 18까지 포함해 주장하면 테스트가 거짓이 된다).
  ...
  - `session=ERROR` → `Blocker` (행 4), `session=STOPPED` → `None` (행 3). `ERROR`는 죽음에서만 나오므로 표 단독 소비자에게 행 4는 backstop이고, §7.3의 `is_alive` 관문이 죽음을 재기동으로 먼저 보내는 것도 함께 고정한다(M-1).
...
- `capability_args`·`prepare_subagents`·`build_session_command`가 어댑터별 기대 argv를 내는 것(Codex `prepare_subagents`는 빈 목록). 역할 아티팩트 명세는 `role_artifacts`가 내고, `start_session`이 `backend.start` 전에 `verify_role_provisioned`(`artifact_root` 기준)로 검사해 부재·불일치면 `RoleNotProvisioned`로 기동을 거부하는 것 — SESSION 역할 fail-closed 회귀(ADR-0017).
...
| Phase 5 | `AgentRunner`, `PlatformAdapter`, `AgentState`, `FakeBackend`, `SessionBackend` ABC | 어댑터 추상 4종(+역할 아티팩트 1종) + 구체 2종(`artifact_root`·`verify_role_provisioned`, ADR-0017) + `submit_key`/`clear_key`, backend `send_key`, runner `submit()`/`clear_input()`/`attach()`, `format_prompt`·`send_prompt` 변경, `start_session(role, ...)` 시그니처 변경, `PLATFORM_MATRIX` 확정 |
...
**상태판정 재설계는 Phase 5로 이월돼 ADR-0016으로 착지했다.** §8.3a 실측이 잠정 마커와 "스트림 꼬리 부분문자열" 판정 모델이 현재 CLI에 부적합함을 밝혀, 훅 기반 상태 방출로의 `detect_state`/`poll_state` 재설계를 Phase 5(공유 계약 소유)가 수행해 ADR-0016으로 확정했다. 그 결과 §8.3a 항목 1~10의 per-item 완료가 Phase 5 판정기 이후로 재-시퀀싱됐다(§8.3a 주). 남은 것은 상태 파일 신선도 정책(하트비트·`ts` 임계)과 라이브 측정뿐이며 슬라이스 B 몫이다. 인계 문서: `WIP/handoff-state-detection-redesign.md`.
...
- **`ERROR`는 프로세스 죽음에서만 나온다(ADR-0016).** 화면 마커 판정이 폐기돼 `poll_state`의 `ERROR`는 죽은 세션(비영 종료 코드거나 `last_error` 표면화)에서만 나온다 — 살아 있는 세션은 `ERROR`가 아니다. §7.3이 결정표(행 4) 전에 `is_alive`로 죽음을 재확인해 재기동으로 보낸다. Watcher·주입·수렴·§4.5 모든 경로에 적용된다 (§4.5, §7.3)
...
- `send_prompt`의 게이트를 `IDLE` 단독으로 좁힌다(Phase 5 슬라이스 A 반영). 제출을 붙인 채 `WAITING_INPUT`을 받으면 권한 프롬프트를 승인하게 되기 때문이다 (§5)
...
- 본 Phase의 Watcher는 `axdt watcher tick` 하나 — 생존 확인·사후 조회·stdout 진단. LLM도, 죽이는 권한도, **쓰는 파일도** 없다. 재기동 근거는 프로세스의 실제 죽음이며, `ERROR`는 그 죽음에서만 나온다(ADR-0016 — 화면 마커 판정 폐기) (§2.4, §6)
...
- 락은 다른 주입자를 배제하지만 **에이전트 자신의 전이는 막지 못한다.** 재폴링과 게이트 사이의 창이 남는다. (ADR-0016으로 `poll_state()`가 캡처 꼬리 마커 추론에서 훅 상태 파일 읽기로 바뀌어 "마커 추론이라 낡는다"던 위험은 닫혔다. 다만 상태 파일 신선도 정책(하트비트·`ts` 임계)은 슬라이스 B까지 잠정이라, 그 정책이 확정되기 전에는 낡은 상태 파일을 읽을 창이 별개로 남는다.) (§4.1)
