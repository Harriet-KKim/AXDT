...
- **격리 정직성:** 본 Phase는 **파일시스템 격리**(D3: 작업본만 마운트)에 더해, 허브 `pre-receive`의 **수신 ref allowlist**(신원-무관, `ADR-0007`)로 비-task ref(`main`·`sot/*`·태그 등) 직접 push와 삭제를 거부한다 — 무인증서 성립하는 **Phase 3 baseline**이다. **콘텐츠·경로 게이트**(`ADR-0007` 규칙 (b), 보호 경로 diff 검사)도 본 Phase에서 구현한다 — `hubgate` 모듈이 신뢰 `main`의 기계용 정책(`rule-protected-paths` 미러, §6.1a)을 읽어 task push의 변경 경로가 보호 경로를 건드리면 거부한다. 남는 것은 **주체별 ref 소유권(누가 어느 task ref를 쓰나) 강제**뿐으로, 이는 인증(SSH/auth)이 필요해 **하드닝 단계로 연기**한다(단일 사용자 신뢰 호스트 가정).
...
- `serve(transport)` → transport는 daemon 단일(그 외 값은 `ValueError`). **선호 포트**(`AXDT_HUB_PORT`/파생)가 **닫혀 있으면** base-path로 `git daemon --pid-file=<daemon.pid> --port=<port>`(receive-pack 허용) 백그라운드 기동 후 **기동 readiness 확인**: `Popen` 자식 생존(`poll() is None` — bind 경합 즉사 감지) + 포트 connect + **우리가 방금 띄운 데몬** 대상 `git ls-remote`(응답성·repo 도달 확인, **timeout 10s**, HEAD 비교 아님). 성공 시 daemon이 `--pid-file`로 기록한 **listener PID**를 확인하고 그 PID가 실제로 우리 데몬으로 해당 포트를 listen하는지 재검증한다(`git daemon`은 `git` 래퍼가 `git-daemon` 자식을 fork하므로 `Popen.pid`=래퍼는 listen하지 않는다 → 기록·판정은 반드시 listener PID로; readiness 성공~기록 사이 외부 포트 선점 TOCTOU도 이 재검증으로 차단). readiness나 재검증이 실패하면 spawn한 자식을 종료하고 `daemon.pid`를 삭제한다(고아 데몬·오기록 방지). 선호 포트가 **열려 있으면** 재사용 여부를 **`daemon.pid` 기반으로 판정**: 기록 PID 생존 + `/proc/<pid>/cmdline`(NUL 구분 argv, `ps -p <pid> -o args=` 폴백)이 기대 `git daemon --base-path=<우리 hub_dir> --port=<port>`와 **원소 단위 일치** + 그 PID가 해당 포트 listen → 우리 것이면 재사용. `daemon.pid` 없음·stale·불일치면 열린 포트를 **외부로 취급**해 파생 포트로 폴백(파생마저 외부 점유면 `RuntimeError`, fail-closed). **금지 ①**: 포트 점유자의 repo HEAD를 비교해 identity를 판정하지 않는다(같은 프로젝트의 다른 워크트리는 HEAD가 동일 → 오식별). **금지 ②**: 정체불명 외부 포트에 wire 프로브(`git ls-remote` 등)를 보내지 않는다(응답 없는 소켓에 영구 블록). 프로세스 조사는 Linux/WSL2 전제(§1 대상 환경).
- `stop_daemon()` → **활성 Leader 세션(`tmux list-windows`)이 있으면 거부**(컨테이너 push 단절 방지), 없을 때만 `daemon.pid` 종료 — 단 종료 전 `serve`와 **동일한 identity 검증**(PID 생존 + `cmdline`이 기대 `git daemon --base-path=<우리 hub_dir>` argv와 일치)을 거쳐 **우리 데몬으로 확인된 PID만 `kill`**(PID 재사용된 무고한 프로세스 종료 방지). (재부팅 후 재기동은 `serve` 멱등 호출로; 자동 재기동 훅은 연기.)
...
- **`install_gate(repo)`(`ADR-0007` (a)+(b)):** 허브 `hooks/pre-receive`에 **얇은 껍데기 훅**을 설치한다 — 설치 시점 인터프리터(`sys.executable`)로 `axdt.infra.hubgate <repo>`를 exec하고, 실행 불가 시 거부(fail-closed). `receive.denyDeletes`·`core.logAllRefUpdates`도 켠다. `init()`이 허브 생성 직후, `serve()`가 daemon 기동 전에 각각 멱등 호출한다. 게이트 판정 로직은 §6.1a `hubgate.py`.

### 6.1a `hubgate.py` — pre-receive 콘텐츠·경로 게이트(`ADR-0007` (a)+(b))
`install_gate`가 심은 껍데기 훅이 `python -m axdt.infra.hubgate <허브 repo>`로 부른다. stdin으로 pre-receive 표준 입력(`<old> <new> <ref>` 줄들)을 받아 각 갱신을 판정하고, 하나라도 위반이면 exit 1(비영 → git이 push 전체 거부), 전부 통과면 exit 0. 거부 사유는 **ASCII 전용** stderr(무인증 wire로 임의 로캘 클라이언트 터미널에 전달 — 현행 훅 규칙 계승).
- **(a) 수신 ref allowlist(이식):** `new`가 zero-SHA(삭제)면 거부, `ref`가 `refs/heads/w<n>.t<n>-<slug>` 정규식 불일치면 거부. 기존 sh 훅 로직을 동작 동일하게 Python으로 옮긴다.
- **(b) 콘텐츠·경로 게이트:** 허용된 task 브랜치 push라도 변경 경로가 보호 경로를 건드리면 거부한다.
  - **비교 기준점:** `base = git merge-base main <new>`; 변경 경로 = `<base>..<new>` diff의 파일 목록(실제 수집 방식은 아래 '변경 경로 수집' 항목 — `-z`). merge-base 실패(공통 조상 없음)면 거부(fail-closed).
  - **정책 소스(신뢰 ref):** `git show main:docs/sot/rule/protected-paths.md`의 기계용 블록을 읽는다 — **후보 브랜치 버전이 아니라 항상 `main` 버전**이라 후보가 검사 규칙을 무력화하지 못한다(`ADR-0007` 대안 D). 블록 존재 시 읽기·파싱 실패는 거부(fail-closed).
  - **규칙 2종:** `deny <glob>` — 변경 경로가 glob에 걸리면 거부. `report-owns <dir>` — `ref`의 **전체 식별자**(`w<n>.t<n>-<slug>`, slug 포함)로 `<dir>` 아래 변경은 `<dir>/w<n>.t<n>-<slug>.md`(그 ref와 **동일 식별자**)만 허용(타 task report 위조 차단; 주체† '누가'는 인증 필요라 연기). 한 경로에 여러 규칙이 걸리면 논리곱으로 판정하며 `deny`가 우선한다.
  - **glob 의미:** SoT 블록 헤더의 문법 정의(`**`=구분자 포함 0개 이상 세그먼트, `*`=세그먼트 내 0개 이상 문자)를 매처가 **구현**하고, 경계 사례(`docs/interim/**/README.md`가 최상위 `docs/interim/README.md`도 매칭; `report-owns` 정확 파일명 일치·하위 디렉터리 거부)를 단위 테스트로 **검증**한다(문법 정의의 정본은 SoT 블록 — 테스트는 검증자이지 오라클이 아니다).
  - **변경 경로 수집:** `git diff -z --name-only <base> <new>`(NUL 구분)로 원시 경로를 받는다 — 기본 `core.quotePath`가 비ASCII 경로를 이스케이프해 glob을 비껴가는 false-accept를 막는다.
- **기계용 정책 블록:** `protected-paths.md`(SoT) 안 코드펜스 ` ```axdt-protected-paths `. 파서·적합성 테스트는 **코드펜스 정보문자열로 이 블록을 선택**한다(같은 파일의 `axdt-critical-paths` 블록과 섞지 않도록 — 파일 전역에서 `deny`/`critical` 줄을 무차별 매칭하지 않는다). 한 줄=규칙 하나, `#` 주석·빈 줄 무시. 사람이 읽는 표의 기계 강제 미러이며 표와의 정합은 적합성 테스트로 결속. **Phase 3 강제 집합:** `docs/sot/**`·`docs/interim/progress.md`·`docs/interim/plan/**`·`docs/interim/sot-readiness-review.md`·`docs/interim/**/README.md`·`docs/interim/**/_TEMPLATE.md`·루트 `README.md`·`LICENSE`·`.gitignore` + `report-owns docs/interim/report`. **제외:** `WIP/**`만(각주 ‡ 도그푸딩 — 대상 프로젝트 코드; 루트 3파일은 무조건 강제). 강제 장치 자체를 보호하는 `axdt-critical-paths` 블록은 Phase 6 머지 게이트 소관이라 hubgate는 읽지 않는다.
- **롤아웃 순서:** 문지기가 `main`의 정책 블록을 읽으므로 블록이 `main`에 반영된 뒤 (b)가 실효한다. 블록 부재는 (a)-only(전환기; `main`은 Maintainer가 out-of-band로만 갱신하고 task push는 (a)가 `main` 갱신을 거부하므로 블록을 task push로 지울 수 없다). 블록이 있으면 파싱/읽기 실패는 fail-closed.
