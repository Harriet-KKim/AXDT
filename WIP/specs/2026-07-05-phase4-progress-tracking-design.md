...
> 상태: **초안 (브레인스토밍 합의, 2차 다중 리뷰 Codex+Fable 반영)** · 작성일 2026-07-05 · 범위: Phase 4 (WIP/TODO.md)
...
- 전이 표는 **Maintainer 편집의 규범**이다. lint는 progress 단일 스냅샷만 봐서 전이 위반을 검사하지 못한다(정적 pair 불변식만 검사, §4). **단 커밋 시점엔 HEAD가 있어 `commit.py`가 행 단위 전이를 검사한다**(§5). 전이 이력은 git(마일스톤 커밋)으로 남는다.
...
5. 비종료가 `todo`뿐(`todo` 또는 `todo`+`superseded`만) → `todo` (미착수 wave — `superseded`가 섞여도 활성 없음)
...
| `commit.py` | HEAD 대비 progress diff로 행 단위 전이 검증(`ALLOWED_TRANSITIONS`, 종료 재개·행 삭제 거부) + diff에서 이벤트·규약 메시지 생성 → 스테이징 → 거부범위 검사 → commit. progress 편집 안 함·수용 판단 안 함·허브 push 안 함(§5·§6.3) | 쓰기(git, 로컬 보조) |
...
- **반려 사유의 durable 기록** = 두 갈래로 남긴다: (a) Leader의 **다음 report 이터레이션**에 "받은 피드백·조치" 반영(report는 Leader 소유), (b) `rejected` 전이의 **마일스톤 커밋 메시지에 사유 요약**(§6.3) — send-keys 휘발성·크래시 소실창 완화(ADR-0002 "git 이력=변경 이력"). (b)는 규범이 아니라 **강제**다: 커밋 메시지는 `commit.py`가 생성하며 `rejected` 이벤트에 사유가 없으면 커밋이 거부된다(§5 `format_milestone_message`). Maintainer 반려 근거 자체의 형식화(별도 산출물)는 Phase 2 사안.
...
- **`report`가 활성인데 `progress=rejected`면 WARN**(재순환 신호) — `in-progress`/`blocked`/`needs-spec` 행의 `rejected` 열. Leader가 아직 되돌아오는 중인 정상 과도기라 ERROR 아님.
- **`blocked` 행 + `progress=paused`는 WARN**(충돌 신호) — Leader는 "외부 의존 대기"를 주장하는데 Maintainer는 "의도적 보류"로 봄. `blocked`≠`paused`(§2.1)라 불일치를 표시.
- **`todo` 행과 `—`(부재) 행의 비대칭**: `report=todo`는 "미착수"를 **적극 주장**하는 것이라 progress가 앞서가도(예: `blocked`/`paused`) 보고 lag일 뿐 정상(·). `report=—`는 주장 자체가 없어, progress가 활성(`in-progress`/`in-review`/`rejected`)이면 WARN. 단 `progress`가 `blocked`/`paused`면 착수 전 정상(·)이고, `accepted`는 주장 없는 수용이라 ERROR.
...
class State:
    ...
    pending_blocker_acceptance: list[...]  # report ∈ {blocked, needs-spec} ∧ progress ∉ {blocked, paused, superseded}
...
#             전이 검증 + diff에서 이벤트·메시지 생성. lint(단일 스냅샷)도 git(의미 무지)도 못 하는 일.
@dataclass
...
def diff_progress(base: list[TaskRow], new: list[TaskRow]) -> list[ProgressEvent]
    # base=HEAD:progress.md, new=작업본. status 변경은 ALLOWED_TRANSITIONS[before] 안이어야 함.
    # TERMINAL(accepted/superseded) 재개·행 삭제 = 오류. 신규 행은 정상경로에서 todo 등록만.
def format_milestone_message(events: list[ProgressEvent],
                             rejection_reasons: dict[str, str] = {}) -> str
...
def milestone_commit(repo: Path, rejection_reasons: dict[str, str] = {},
    ...
    # diff_progress 전이 검증 → 거부범위 검사(아래) → progress.md(+관련 report) 스테이징
    # → 생성 메시지로 commit. progress 편집 안 함·수용 판단 안 함·허브 push 안 함.
    #
    # 거부범위(Maintainer가 지금 progress.md에서 고칠 수 있는 문제만 거부):
    #   (1) HEAD→작업본 diff의 불법 변경(비허용 전이·종료 재개·행 삭제)
    #   (2) progress.md 구조 오류(§4.1)
    #   (3) 이번 커밋에서 올린 행의 과claim(그 행을 accepted/in-review로 바꾸는데 그 task report=done 미확인)
...
2. 도구가 progress(권위)+report를 재구성해 **명시적 집합**으로 분류(§5 `State`): 수용 대기(`report=done ∧ progress ∉ {accepted, superseded}`), 블로커/사양 수용 대기(`report ∈ {blocked, needs-spec} ∧ progress ∉ {blocked, paused, superseded}`), 재작업 중(`progress=rejected`), 블로커/보류(`progress ∈ {blocked, paused}`), 주의 필요(§4.3 매트릭스에서 파생), wave 롤업. (순서 비교 없이 집합 소속으로만 판정 — 드리프트 방지.)
...
- **누가**: Maintainer(호스트 상시 세션, ADR-0001). `commit.py`가 **HEAD 대비 전이 검증** → 거부범위 검사 → 스테이징 → **도구가 생성한** 메시지로 commit(§5).
- **전이 검사**: 커밋 시점엔 HEAD와 작업본이 둘 다 있어 `diff_progress`가 행 단위 전이를 `ALLOWED_TRANSITIONS`로 검증한다(lint가 단일 스냅샷이라 못 하는 검사). 종료 재개·행 삭제·비허용 전이는 거부.
- **거부범위**: (1) diff의 불법 변경 (2) progress.md 구조 오류(§4.1) (3) 이번 커밋에서 올린 행의 과claim(그 task report=done 미확인)만 거부. 이번에 손 안 댄 다른 task의 report 문제는 **경고+라우팅**(막으면 데드락). 상세는 §5 `milestone_commit` 주석.
- **메시지 규약**: 도구가 diff에서 **생성**한다(자유 `-m` 아님). 형식 — subject `chore(progress): <task> <from>-><to>`, 복수/wave면 `chore(progress): batch <n> events` + 본문 `Events:` 목록. `rejected` 이벤트는 본문에 `Reason: <task> <사유>` 필수(§2.4 durable 기록 (b)).
...
- `axdt progress status` — recover 요약(수용 대기·블로커 수용 대기·재작업·블로커·wave 롤업).
...
- 커버: **정합 매트릭스 §4.3 전 셀**(report 6상태 × progress 8상태), 구조·형식 검사(§4.1), 참조 무결성(§4.2 고아 report·비-task 파일 제외·id/status 검사), `wave_rollup` 전량 사상(빈 wave·all-superseded·`todo`+`superseded`·혼재·우선순위), 파싱 라운드트립, recover 집합 분류(수용 대기·블로커 수용 대기·재작업).
- commit: **전이 거부**(비허용 전이·종료 재개·행 삭제), **diff→이벤트 파생**(수용·반려·wave·등록·복합), **생성 메시지 파싱 라운드트립**(`rejected` 사유 필수), **거부범위 동작**(이번 커밋 행의 과claim은 거부 vs 남의 report 문제는 통과).
