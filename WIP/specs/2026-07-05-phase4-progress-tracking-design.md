...
- **현재 사실(중요):** 허브(`WIP/axdt/infra/hub.py`)는 pre-receive 게이트로 **(a) 수신 ref allowlist**(task 브랜치 형식만 push 허용, 그 외 ref·삭제 거부; `hub.install_gate`)를 강제한다. 그러나 **보호 경로 diff를 거부하는 (b) 경로/콘텐츠 게이트는 아직 미구현**이다 — ADR-0007상 "하드닝"이 아니라 **"Phase 3 baseline"**이며 그 검사 코드가 Phase 3 후속 잔여다. 하드닝으로 연기된 것은 **주체 인증(ref 위장 방지)뿐**이다. `ADR-0007`은 `status: proposed`(그 (b) 완료까지).
