...
## 규칙문
> `docs/sot/`(requirements·specification·test-design·rule)의 변경은 **사용자를 Reviewer로 둔 PR**을 통해서만 `main`에 머지된다. Agent는 SoT를 **`main`에 직접(PR 없이) 커밋하지 않으며**, 변경이 필요하면 **Maintainer가 `sot/<slug>` 브랜치에서 PR을 생성**하고 **사용자 승인까지 일시정지**한다. 승인·머지되면 재개하고, 반려되면 PR을 수정해 다시 게이트를 거친다.
...
## 적용범위
- **대상**: `docs/sot/` 전체. 그중 **요구·사양·테스트설계 저술은 Author**가 하고(`rule/` 변경은 Author 저술 범위가 아니라 설계·거버넌스 결정으로 다룬다), 어느 경우든 **PR 발의(생성)는 Maintainer만** 한다(구현 중 필요한 변경은 Leader의 사양변경요청 → Maintainer가 PR화). Author의 저술 산출도 `sot/<slug>` PR을 거쳐 사용자 게이트에 오르며, 누구도 SoT를 **`main`에 직접(게이트 없이) 반영하지 않는다**. Leader는 task 브랜치에서 `docs/sot/` 수정 자체가 거부된다(`rule-protected-paths`).
...
## 예시
**준수 (✓)**
- Author가 요구·사양·테스트설계 초안을 저술 → Maintainer가 `sot/` PR 발의, 사용자 승인 대기.
