...
# 진행 기록은 Maintainer만 작성한다 (단일 작성자)
...
## 근거
- progress는 "시스템이 참으로 수용한 상태"를 담는 **권위 있는 진행 기록**이다. 작성자가 여럿이면 동시 기록 경합·상호 덮어쓰기로 권위가 깨진다.
...
## 적용범위
- **대상**: `docs/interim/progress.md`의 **내용·상태 기록/갱신**(모든 대상 프로젝트; report는 canonical 경로 `docs/interim/report/<task>.md`로 판정 — progress 컬럼 아님). 쓰기 권한은 Maintainer 역할에 한정. (초기 빈 양식/스캐폴딩 생성은 Phase 0·1의 1회성 셋업으로 본 규칙과 별개.)
...
**위반 (✗)**
- Leader가 `progress.md` 에서 자기 task 행을 직접 `done` 으로 고침. → 권위 있는 진행 기록을 우회한 자기수용. 다른 Leader 기록과 경합하고 Maintainer 게이트를 건너뛴다.
