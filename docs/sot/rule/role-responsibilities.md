...
status: active
scope: global
related: [rule-protected-paths, rule-subagent-no-direct-communication, rule-progress-single-writer, rule-report-to-progress-authority, rule-sot-change-user-gate, rule-terminology, ADR-0007]
---
...
⁵ **ADR 기록은 Maintainer가 한다.** `docs/interim/ADR/*.md`(ADR 본문)의 작성 주체는 Maintainer다. Leader는 진행 중 내린 설계 결정을 ADR로 남길 필요가 있으면 자기 report로 **제안**하고, Maintainer가 기록한다 — `plan`과 같은 "제안(Leader) → 기록(Maintainer)" 패턴이다(스펙 §2.6). ADR 폴더의 `README.md`·`_TEMPLATE.md`도 `maintainer` 행의 `docs/interim/**/README.md`·`docs/interim/**/_TEMPLATE.md`가 소유한다. (초안은 ADR 본문을 `leader` 쓰기 경로에 뒀으나, plan과 같은 자가수정·조율 사유로 작성 권한을 Maintainer로 옮겼다.)
