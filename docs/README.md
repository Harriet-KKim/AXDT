...
> AXDT의 모든 프로젝트 문서가 모이는 루트. **SoT(권위본)** 와 **interim(중간 산출물)** 을 구분하고, 그 위에 사람을 위한 **길잡이(guide)** 문서를 둔다.
...
- **`glossary.md`·`workflow.md`** — 사람을 위한 **길잡이(guide)** 문서. SoT도 interim도 아닌 비권위 안내로, `docs/` 바로 아래 둔다. 정의·흐름의 권위는 이들이 링크하는 SoT·ADR 원본에 있다.
...
## 참고
- 전체 용어 사전: `glossary.md`
- 문서·작업 워크플로 개관: `workflow.md`
