...
> 요구사항·사양·테스트 설계 문서를 Source of Truth로 두면, AI 에이전트들이 그 문서로부터 소프트웨어를 개발한다.

이 프로젝트는 AI Agent를 통한 개발을 적용할 수 있는 템플릿입니다.
Claude Code(`.claude/`), Codex(`.codex/`) 모두 지원합니다.

요구사항, 사양, 테스트 설계는 Agent와 함께 정해진 Skill을 통해 작성하며
이후 문서 작성이 완료되면 자동으로 프로젝트 개발을 진행합니다.

개발 진행 과정은 모두 Maintainer에 의해 수행되며, 기본적으로 Git을 통한 이력 관리가 이뤄집니다.
요구사항·사양·테스트 설계 변경, 회색지대의 결정 사항 등 사용자의 의사 결정이 필요한 경우에는 사용자가 Reviewer로 지정된 PR 생성 후 일시 중지됩니다.
...
docs/
  sot/                 # Source of Truth — 변경은 사용자 게이트 PR로만
    ...
    test-design/       # 어떻게 검증 (테스트 조건·커버리지·스위트·추적·절차 원칙)
...
2. `docs/sot/`의 `requirements/`·`specification/`·`test-design/`에 요구사항·사양·테스트 설계를 작성한다(전용 Skill은 구축 중).
3. 문서가 완료 기준을 충족하면 Maintainer가 개발을 시작한다 — Leader를 격리 workspace·컨테이너에 배치하고 tmux로 조율한다.
...
| **git** | 이력 관리·격리(workspace clone)·통합 허브 | 2.x (권장 최신) |
| **Docker** | Leader 격리 — workspace당 컨테이너 1개 (D3) | 권장 최신 |
...
- **Leader task 브랜치** — `branch-workspace-naming` 규칙의 단일 식별자 `w<n>.t<n>-<slug>`를 branch·workspace·컨테이너에 일관 적용(`docs/sot/rule/branch-workspace-naming.md`).
