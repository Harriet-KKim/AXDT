...
| **Leader** | 기능(task) 단위 개발, 격리 작업본에 종속, report로 자기보고 |
| **Developer / Reviewer / Tester** | Leader가 작업본 내부에서 호출하는 하위 역할 |

역할 간 통신은 **Maintainer를 허브로** 이뤄진다 — Maintainer→Leader는 tmux 하향 주입, Leader→Maintainer는 report(상향). Leader끼리는 직접 통신하지 않고 Maintainer를 경유한다. (`docs/sot/rule/` 참조)
...
WIP/                   # AXDT 자체 설계·구현 (ADR, TODO, axdt 패키지)
```
...
3. 문서가 완료 기준을 충족하면 Maintainer가 개발을 시작한다 — Leader를 격리 작업본·컨테이너에 배치하고 tmux로 조율한다.
...
| **git** | 이력 관리·격리(cloned-worktree)·통합 허브 | 2.x (권장 최신) |
| **Docker** | Leader 격리 — 작업본당 컨테이너 1개 (D3) | 권장 최신 |
...
- **phase/기능 브랜치** — 모든 작업은 여기서 한다(예: `phase0`). 병렬 세션은 각자 브랜치/작업본을 쓴다.
- **Leader task 브랜치** — `branch-worktree-naming` 규칙의 단일 식별자 `w<n>.t<n>-<slug>`를 branch·작업본·컨테이너에 일관 적용(`docs/sot/rule/branch-worktree-naming.md`).
