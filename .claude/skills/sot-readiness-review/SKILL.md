...
description: SoT의 requirements와 specification이 개발 착수에 충분한지 정합성·공백을 항목 단위로 검토·판정하고 그 근거를 docs/interim/sot-readiness-review.md에 감사 로그로 남긴다. rule-sot-readiness의 ② 검토 단계에서 사용한다.
---

# SoT Readiness Review

`rule-sot-readiness`의 **② 정합성·공백 검토**를 수행한다. requirements와 specification을 읽고 항목 단위로 대조해, 개발을 시작하기에 부족한 지점을 찾아 판정하고 감사 로그로 남긴다.
...
## 입력
- 검토 대상 커밋(SoT ref)과 그 커밋의 `docs/sot/requirements/`·`docs/sot/specification/` 전체.
- 직전 회차 감사 로그(있으면) — finding 이월·상태 승계용.
- `related`·`covers`·`rules` 필드·파일명은 **힌트로만** 쓴다. requirements와 specification은 다대다이고 분해 축이 다를 수 있으므로, 짝은 내용을 읽어 판단한다. (`rules`는 rule 변경 재검토의 대상 선정에 쓰되, 충돌 판단 자체는 내용으로 한다.)

## 검토 4축
1. **커버리지·추적성** — 각 요구 항목(`FR-n`·`NFR-n`)이 하나 이상의 사양 항목(`SP-n`)으로 구체화되는가. 구체화되지 않은 요구, 대응 요구가 없는 사양(고아 spec)을 찾는다.
2. **짝 정합성** — 요구와 그 사양이 서로 모순되지 않는가(수치·조건·용어).
3. **교차 정합성** — 사양들 사이, 사양과 rule/기존 SoT 사이에 충돌이 없는가. 이때 문서가 **`rules`에 선언하지 않은 rule에 실제로 의존**함을 발견하면(선언 완전성) finding으로 올린다 — 재검토 타겟팅이 `rules` 자기신고에 의존하므로, 누락은 낡은 완료를 낳는다.
4. **완결성·공백** — 결정이 빠졌거나 모호한 지점(수용 기준의 무의미한 filler 포함), 고려되지 않은 예외·실패 모드.
...
## 재검토 범위 (수렴 보장)
- **축1(커버리지·추적성)은 매 회차 전량 재생성**한다 — 저비용이고, 변경이 기존 항목과 새 공백·고아를 만들 수 있어서다.
- **축2·4(짝·완결성 심층 판단)만** 2회차부터 변경분 + 직전 회차의 미해결(`open`) finding으로 범위를 좁힌다. 회차마다 새 지적이 무한히 나와 종결되지 않는 상태를 피한다.
- **축3(교차 정합성)**은 변경분 + 그 변경이 참조하거나 참조받는 문서(관련 rule 포함)로 범위를 잡는다 — 국소 변경이 원거리 문서와 충돌할 수 있어 인접까지 본다. **rule 변경으로 인한 재검토**는 이 축3에 한정하며, 대상은 바뀐 rule을 따르는 req·spec이다(`scope: local`은 `rules`를 선언한 문서, `scope: global`은 완료된 전량; 신규 rule·`scope` 전환 시에는 declared scope와 무관하게 완료 전량 축3 스윕 — `rule-sot-readiness` 적용범위).
...
2. **커버리지 표** — 요구↔사양 양방향.
   ...
   | 요구 항목 | 사양 항목 | 요구사항 문서 | 사양 문서 | 커버리지 |
   |---|---|---|---|---|

   커버리지 값: `covered`(요구 항목이 사양으로 충분히 구체화) · `partial`(일부만, 빠진 측면 있음) · `none`(대응 사양 없음). 비기능 요구(`NFR-n`)는 하나의 `SP-n`이 아니라 여러 사양에 걸치거나 별도 검증으로 갈 수 있으므로, 대응 사양을 복수로 적거나 검증 방식을 참조로 표기한다. 대응 요구가 없는 사양은 요구 항목을 `—`(고아)로 둔 행으로 표기한다.
